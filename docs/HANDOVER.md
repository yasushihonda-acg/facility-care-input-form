...
> **最終更新**: 2025年12月21日（Phase 30.1完了・通知機能拡張）
>
...
| **記録閲覧ソート改善** | 日時カラムのソートを正確な時系列順に修正（Phase 23） | ✅ 完了 |
| **カラムリサイズ機能** | テーブルビューのカラム幅をドラッグで調整可能に（Phase 24） | ✅ 完了 |
| **全文検索機能** | テーブルビューでの全文検索フィルタリング（Phase 25） | ✅ 完了 |
| **入居者設定削除** | 禁止ルールUIを一時非表示、将来プリセットに統合予定（Phase 26） | ✅ 完了 |
...
| **家族操作・入力無し通知** | 品物登録/編集/削除通知、設定可能時刻の入力無し警告（Phase 30/30.1） | ✅ 完了 |
| **カテゴリ簡素化** | 品物カテゴリを7→2に簡素化（食べ物/飲み物）、スタッフ記録タブ固定化（Phase 31） | ✅ 完了 |
| **統計ダッシュボード品物別分布** | カテゴリ別→品物別分布に変更、具体的な消費傾向可視化（Phase 32） | ✅ 完了 |
| **残ったものへの処置指示** | 家族が事前に「破棄/保存」を指定、スタッフ記録時にガイド表示（Phase 33） | ✅ 完了 |
| **AI分析デモモード対応** | 統計ダッシュボードのAI分析がデモモードで動作（Phase 34） | ✅ 完了 |
...
| **Phase 30.1** | **通知機能拡張**（品物削除通知追加、記録チェック時刻を設定可能化、Cloud Scheduler毎時実行） | 2025-12-21 |
| **Phase 31** | **カテゴリ簡素化**（7→2カテゴリ、スタッフ記録フォームタブ固定化、migrateCategory後方互換） | 2025-12-21 |
...
| `firebase-schedule-checkDailyRecords-asia-northeast1` | 毎時0分 (Asia/Tokyo) | 設定時刻に入力無しチェック（Phase 30.1で毎時実行に変更） |

...
> **最終更新**: 2025年12月20日 (Phase 19.1 記録の自動反映 - 実装完了・本番デプロイ済み)
>
...
### Phase 19: 記録のチャット連携（Phase 19.1 完了）
...
| Phase | 内容 | 工数 | 状態 |
|-------|------|------|------|
| 19.1 | バックエンド - submitMealRecord拡張 | 2-3h | ✅ 完了 |
| 19.2 | フロントエンド - RecordMessageCard | 2-3h | ✅ 完了 |
| 19.3 | ホーム通知セクション | - | ⏳ 後日（Phase 19.2） |
| 19.4 | E2Eテスト・デプロイ | 1-2h | ✅ 完了 |
| **合計** | | **5-7h** | **Phase 19.1 完了** |
...
**主要機能**:
- ✅ **記録の自動反映** - 提供記録がスレッドに自動表示（Phase 19.1）
- ✅ **RecordMessageCard** - カード形式で記録を表示（Phase 19.1）
- ⏳ **ホーム通知** - 新着メッセージ・記録をホームに表示（Phase 19.2）

**E2Eテスト**: 8件追加（`record-chat-integration.spec.ts`）

**次のステップ**: Phase 19.2（ホーム通知セクション）または他のPhaseを選択
...
- チャット一覧はアクティブのみ表示（視認性向上）
- フッターにチャットアイコン追加（独立したビューとしてアクセス可能）
- Phase 18: チャット機能の基本（一覧・送受信・通知）を優先
...
| Phase | 内容 | 工数 | 状態 |
|-------|------|------|------|
| 18.1 | データモデル・API基盤 | 4-6h | ✅ 完了 |
| 18.2 | チャットUI基盤（ChatListPage含む） | 8-10h | ✅ 完了 |
| 18.3 | 通知システム（フッター改修含む） | 4-5h | ✅ 完了 |
| 18.4 | 家族側チャット連携 | 4-5h | ✅ 完了 |
| 18.5 | デモモード対応・テスト | 3-4h | ✅ 完了 |
| **合計** | | **23-30h** | **完了** |
...
| `functions/src/index.ts` | チャットAPI export追加 |
...
| `frontend/e2e/chat-integration.spec.ts` | E2Eテスト（新規） |

**主要機能**:
- ✅ **チャット一覧（ChatListPage）** - アクティブなスレッドのみ表示
- ✅ 品物チャットスペース（スタッフ⇔家族の双方向メッセージ）
- ✅ **品物詳細にチャット開始ボタン** - 品物詳細画面からチャットを開始
- ✅ 通知バッジ（フッター）・未読カウント表示
- ✅ デモモード対応・E2Eテスト

**次のステップ**: Phase 19（ホーム通知・記録の自動反映）の設計・実装
...
**概要**: スタッフがアップロードした写真を家族画面（エビデンス・モニター）で表示する機能。デモモードではダミー画像を表示。

**変更点**:

| 変更前 | 変更後 |
|--------|--------|
| プレースホルダ（📷アイコン） | 実画像表示（photoUrlから取得） |

**実装ファイル**:

| ファイル | 内容 |
|----------|------|
| `EvidenceMonitor.tsx` | プレースホルダ → 実画像表示に変更 |
| `TimelineItem.tsx` | リンクをデモモード対応 |
| `App.tsx` | `/demo/family/evidence/:date` ルート追加 |
| `demoFamilyData.ts` | ダミー画像URL追加（picsum.photos） |
...
### Phase 15.6: 摂食割合の数値入力・残り対応フィールド（完了）

**設計書**: [STAFF_RECORD_FORM_SPEC.md](./STAFF_RECORD_FORM_SPEC.md) セクション9

**概要**: 摂食状況の入力方式を絵文字ボタン（😋完食など）から0-10の数値入力に変更。さらに、完食でない場合（10未満）に「残った分への対応」を記録できるフィールドを追加。

**変更点**:

| 変更前 | 変更後 |
|--------|--------|
| 絵文字5択（完食〜なし） | 数値入力（0-10） + スライダー |
| なし | 残った分への対応（条件付き表示） |
...
| ファイル | 内容 |
|----------|------|
| `consumptionLog.ts` | `RemainingHandling`型・選択肢追加 |
| `mealForm.ts` | `SnackRecord`型更新 |
| `StaffRecordDialog.tsx` | 数値入力UI、残り対応フィールド追加 |
| `staff-record-form.spec.ts` | E2Eテスト4件追加（STAFF-040〜043） |

**E2Eテスト**: 22件（Phase 15全体）
...
**概要**: スタッフ用記録入力フォームを簡素化・統一。食事タブを削除し「品物から記録」のみに。家族連絡詳細からのダイアログ表示も統一。

**実装内容**:

| Phase | 内容 | 状態 |
|-------|------|------|
| **Phase 15.1** | タブ削除・フォーム項目整理 | ✅ 完了 |
| **Phase 15.3** | 家族連絡詳細のダイアログ化 | ✅ 完了 |
| **Phase 15.4** | API連携・Sheet B対応 | ✅ 完了 |
| **Phase 15.6** | 摂食割合の数値入力・残り対応 | ✅ 完了 |
...
|------|------|------|
| 入力者名 | ✅ | スタッフ名 |
| デイサービス利用 | ✅ | 利用中/利用中ではない |
| デイサービス名 | 条件付き | 利用中の場合のみ |
| 品物選択・提供記録 | - | 品物リストから選択 |
| 摂食した割合 | ✅ | 0-10の数値入力（Phase 15.6） |
| 残った分への対応 | 条件付き | 摂食割合10未満の場合（Phase 15.6） |
| 間食について補足 | - | 自由テキスト |
| 特記事項 | - | ケアに関すること |
| 重要特記事項 | ✅ | 集計表反映フラグ |
| 写真 | - | アップロード |
...
| `StaffRecordDialog.tsx` | 統一ダイアログコンポーネント |
| `FamilyMessageDetail.tsx` | StaffRecordDialog使用 |
| `ItemBasedSnackRecord.tsx` | StaffRecordDialog使用 |
| `staff-record-form.spec.ts` | E2Eテスト22件 |

**E2Eテスト**: Phase 15全体で22件

**デプロイ**: GitHub Actions経由でHostingデプロイ完了、本番環境でE2E検証済み

---

### Phase 14: スタッフ用デモページ（完了）

**設計書**: [DEMO_STAFF_SPEC.md](./DEMO_STAFF_SPEC.md)

**概要**: デモモードにスタッフ向けページを追加。家族用デモ（/demo）と並行して、スタッフ視点でのデモ体験（/demo/staff）を提供。シードデータは家族用と共有し、効率的な実装を実現。

**実装内容**:

| ファイル | 内容 |
|----------|------|
| `DemoStaffHome.tsx` | スタッフ用デモホーム（4機能カード） |
| `DemoStaffShowcase.tsx` | スタッフ用ガイドツアー（4ステップ） |
| `App.tsx` | スタッフデモルート有効化 |
| `FooterNav.tsx` | /demo/staff/*でスタッフフッター表示 |
| `DemoHeaderButton.tsx` | スタッフデモ用ツアーTOPボタン（緑色） |
| `DemoHome.tsx` | スタッフ用デモへのリンク追加 |
| `demo-staff.spec.ts` | E2Eテスト17件 |

**ルート構成**:

| パス | 説明 |
|------|------|
| `/demo/staff` | スタッフ用デモホーム |
| `/demo/staff/showcase` | ガイドツアー |
| `/demo/staff/family-messages` | 家族連絡一覧 |
| `/demo/staff/family-messages/:id` | 品物詳細 |
| `/demo/staff/input/meal` | 食事記録入力 |

**E2Eテスト**: 161件パス（+17件追加）、3件スキップ（本番データ必要）

**デプロイ**: GitHub Actions経由でHostingデプロイ完了、本番環境でE2E検証済み

---

### Phase 13: 品物起点の間食記録・スケジュール拡張

**設計書**: [ITEM_BASED_SNACK_RECORD_SPEC.md](./ITEM_BASED_SNACK_RECORD_SPEC.md)

**概要**: スタッフが品物情報を見ながら直感的に間食記録を入力できる機能と、家族がより柔軟な提供スケジュールを設定できる機能。
...
| **Phase 13.1** | スケジュール拡張（家族用） | ✅ 完了 |
| Phase 13.1.1 | 型定義・ユーティリティ | ✅ 完了 |
| Phase 13.1.2 | スケジュール入力UI | ✅ 完了 |
| Phase 13.1.3 | API・Firestore対応 | ✅ 完了 |
| Phase 13.1.4 | 品物から記録タブとの統合 | ✅ 完了 |
| Phase 13.1.5 | E2Eテスト | ✅ 完了 |
| **Phase 13.2** | スタッフ向けスケジュール表示強化 | ✅ 完了 |
| Phase 13.2.1 | scheduleUtils拡張・ScheduleDisplay | ✅ 完了 |
| Phase 13.2.2 | WeekdayBadgesコンポーネント | ✅ 完了 |
| Phase 13.2.3 | ItemCard統合・E2Eテスト | ✅ 完了 |
...
**スケジュールタイプ（Phase 13.1）**:

| タイプ | 説明 | 表示例 |
|--------|------|--------|
| once | 特定の日 | `12/20 おやつ時` |
| daily | 毎日 | `毎日 おやつ時` |
| weekly | 曜日指定 | `月・水・金 おやつ時` |
| specific_dates | 複数日指定 | `12/20, 12/22, 12/25 おやつ時` |

**次のステップ**: Phase 14完了、次のフェーズを計画中

---

## 最近の完了タスク

### Phase 13.2: スタッフ向けスケジュール表示強化 (2025-12-19)

**設計書**: [ITEM_BASED_SNACK_RECORD_SPEC.md](./ITEM_BASED_SNACK_RECORD_SPEC.md) セクション4

**概要**: Phase 13.1で家族が設定したスケジュール情報を、スタッフ側の「品物から記録」タブでより分かりやすく表示する機能。

**実装内容**:

| Phase | 内容 | 実装ファイル |
|-------|------|-------------|
| 13.2.1 | scheduleUtils拡張 | `scheduleUtils.ts` に表示用ヘルパー追加 |
| 13.2.2 | WeekdayBadges | `WeekdayBadges.tsx` 曜日バッジコンポーネント |
| 13.2.3 | ScheduleDisplay | `ScheduleDisplay.tsx` スケジュール表示統合 |

**改善ポイント**:

| 項目 | 改善内容 |
|------|----------|
| タイムスロット | 「おやつ時」「朝食時」など提供タイミングを表示 |
| 曜日バッジ | 今日の曜日をハイライト表示 |
| 次回予定日 | 今日が該当しない場合「次回: 12/23（月）」を表示 |
| 補足メモ | スケジュール補足メモを表示 |

**E2Eテスト**: `schedule-display.spec.ts`（7件）、全144件パス

---

### Phase 13.1: スケジュール拡張（家族用）(2025-12-19)

**設計書**: [ITEM_BASED_SNACK_RECORD_SPEC.md](./ITEM_BASED_SNACK_RECORD_SPEC.md) セクション3

**概要**: 家族が品物登録時に柔軟なスケジュールを設定できる機能。単一日付の `plannedServeDate` を拡張し、毎日・曜日指定・複数日指定に対応。後方互換性を維持。

**実装内容**:

| Phase | 内容 | 実装ファイル |
|-------|------|-------------|
| 13.1.1 | 型定義・ユーティリティ | `careItem.ts`, `scheduleUtils.ts` |
| 13.1.2 | スケジュール入力UI | `ServingScheduleInput.tsx`, `WeekdaySelector.tsx`, `MultipleDatePicker.tsx` |
| 13.1.3 | API・Firestore対応 | `functions/src/types/index.ts` |
| 13.1.4 | 品物から記録タブ統合 | `ItemBasedSnackRecord.tsx`, `ItemForm.tsx` |
| 13.1.5 | E2Eテスト | `schedule-extension.spec.ts`（7件） |

**スケジュールタイプ**:

| タイプ | UI | 表示例 |
|--------|-----|--------|
| once | 日付選択 | `12/20 おやつ時` |
| daily | タイプ選択のみ | `毎日 おやつ時` |
| weekly | 曜日トグル（日〜土） | `月・水・金 おやつ時` |
| specific_dates | 日付追加/削除 | `12/20, 12/22 おやつ時` |

**提供タイミング（timeSlot）**: 朝食時、昼食時、夕食時、おやつ時、いつでも

**後方互換**:
- `servingSchedule.type === 'once'` の場合は `plannedServeDate` に同期
- 旧データは `isScheduledForToday()` で引き続き正しく判定
...
**概要**: スタッフが品物情報を見ながら直感的に間食記録を入力できるUI実装。

**実装内容**:

| ファイル | 内容 |
|----------|------|
| `frontend/src/components/meal/MealInputTabs.tsx` | 食事/品物から記録タブ切替コンポーネント（新規） |
| `frontend/src/components/meal/ItemBasedSnackRecord.tsx` | 品物リスト（グループ化・ソート）（新規） |
| `frontend/src/components/meal/SnackRecordModal.tsx` | 記録入力モーダル（新規） |
| `frontend/src/pages/MealInputPage.tsx` | タブ切替統合（既存改修） |
| `frontend/e2e/item-based-snack-record.spec.ts` | E2Eテストスケルトン（新規） |

**機能**:

| 機能 | 説明 |
|------|------|
| **タブ切替** | 食事タブ↔品物から記録タブの切替 |
| **品物グループ化** | 今日提供予定/期限が近い/その他の3グループに分類 |
| **ソート** | 期限順→送付日順で並び替え |
| **記録モーダル** | 提供数・摂食状況・メモ入力、consumptionLog API連携 |
| **サジェスト** | 家族指示から推奨提供数を自動計算 |

**残タスク**: Phase 13.0.4でSheet B書き込み連携を実装予定

---

### Phase 12.1: 手動登録時プリセット保存提案 (2025-12-18)

**設計書**: [PRESET_MANAGEMENT_SPEC.md](./PRESET_MANAGEMENT_SPEC.md) セクション5.4, 9

**概要**: 品物を手動登録した際にも、AI提案時と同様にプリセット保存を提案するダイアログを表示。

**実装内容**:

| ファイル | 内容 |
|----------|------|
| `frontend/src/components/family/SaveManualPresetDialog.tsx` | 手動登録保存ダイアログ（新規） |
| `frontend/src/pages/family/ItemForm.tsx` | 登録完了後ダイアログ表示ロジック追加 |

**機能**:

| 機能 | 説明 |
|------|------|
| **登録後ダイアログ** | 品物登録完了後にプリセット保存を提案 |
| **入力内容表示** | 品物名、カテゴリ、提供方法、詳細を表示 |
| **プリセット名自動生成** | 「品物名（提供方法）」形式で自動生成 |
| **デモ/本番両対応** | デモモードでは実際の保存は行わない |

**E2Eテスト**: 117件全パス（既存テストへの影響なし）
...
| 機能 | 説明 | 優先度 |
|------|------|--------|
| **Phase 19: 記録のチャット連携** | 提供記録がチャットスレッドに自動反映 | 中 |
| 週次レポート生成（aiReport） | Gemini連携による週次サマリー自動生成 | 低 |
| ケア指示のFirestore保存 | モックデータ → Firestore永続化 | 低 |
| CSVエクスポート | 表示中のデータをCSVでダウンロード | 低 |

> **注**: 写真エビデンス表示機能は Phase 16/17 で実装完了済み（Firebase Storage連携）
>
> **注**: Phase 19 は Phase 18 完了後に着手（設計書: [CHAT_INTEGRATION_SPEC.md](./CHAT_INTEGRATION_SPEC.md) セクション6）
...
Phase 13.0: 品物起点の間食記録      ████████████████████ 100% (完了)
Phase 13.1: スケジュール拡張        ████████████████████ 100% (完了)
Phase 13.2: スケジュール表示強化    ████████████████████ 100% (完了)
Phase 14: スタッフ用デモページ      ████████████████████ 100% (完了)
Phase 15: 記録入力フォーム統一      ████████████████████ 100% (完了)
```

...
> **最終更新**: 2025年12月17日 (ツアーナビゲーションUX改善)
>
...
**解決策**: ヘッダー右側に目立つ「ツアー」ボタンを常時表示

**実装内容**:
- `DemoHeaderButton.tsx`: ヘッダー右側に表示するオレンジ色のボタン（新規）
- `Layout.tsx`: ヘッダー内にDemoHeaderButtonを配置
- `TourReturnBanner.tsx`: 削除（不要になった）
...
**E2Eテスト**:
- `demo-page.spec.ts`: ツアーナビゲーションテスト8件（スクロールテスト追加）
- **検証**: 全43件のデモテストがパス

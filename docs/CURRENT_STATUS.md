...
> **最終更新**: 2025年12月13日 (Phase 4.7 テーブルビュー表示カラム設計)
>
...
1. **設計方針**
   - 一覧性の重視（スクロールなしで重要情報を把握）
   - シート目的への最適化（各シートの記録目的に応じた主要カラムを優先表示）
   - モバイルファースト（4〜6カラム程度に絞り、横スクロールを最小化）

2. **11シート全ての表示カラム設計**
   - 食事: 時間帯、主食摂取量、副食摂取量を優先
   - 水分摂取量: 水分量(cc)を優先
   - 排便・排尿: 排便有無、排尿有無、排尿量を優先
   - バイタル: 体温、血圧(高/低)、脈拍、SpO2を優先
   - 口腔ケア: 時間帯を優先
   - 内服: タイミング、頓服時刻を優先
   - 特記事項: 重要度、内容プレビュー(50文字)を優先
   - 血糖値インスリン投与: 測定時間、血糖値、投与時間、投与量を優先
   - 往診録: 担当医、次回往診、指示変更有無を優先
   - 体重: 体重(kg)を優先
   - カンファレンス録: 内容プレビュー、対応事項を優先

3. **実装仕様の定義**
   - TypeScript型定義（SheetColumnConfig, ColumnDef）
   - バッジ表示設定（重要度、指示変更の視覚的表示）
   - 文字数制限による省略表示

### 次のタスク

**Phase 4.7 テーブルビュー表示カラム実装**:
- `frontend/src/config/tableColumns.ts` の作成
- `HomePage.tsx` のテーブル表示ロジック修正
- シート別カラム表示の実装
- バッジ表示・文字数省略の実装

**Phase 4.6 差分同期実装（オプション）**:
- 自動同期をincremental（差分取得）化してコスト削減
- ただし現状はフロントエンドsetIntervalのため、ユーザー非稼働時はコスト発生なし
...
Phase 4.7: テーブルビュー表示カラム ██████░░░░░░░░░░░░░░  30% (設計完了・実装待ち)
```

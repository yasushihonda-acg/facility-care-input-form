...
1. **家族**: 期限切れアラートで「廃棄」ボタン押下
2. **システム**: ステータスを `pending_discard` に変更
3. **家族側**: 「スタッフに通知中...」と表示
4. **スタッフ側**: 注意事項ページの「家族依頼」タブにバッジ表示
5. **スタッフ**: 廃棄指示を確認し「廃棄完了」ボタン押下
6. **システム**: ステータスを `discarded` に変更
7. **両側**: 品物が一覧から消える
...
| ステータス | 説明 | 家族側表示 |
|------------|------|-----------|
| `pending_discard` | 廃棄指示中 | 「通知中...」（グレー背景） |
| `discarded` | 廃棄完了 | 一覧から消える（破棄済みタブに表示） |

### 8.4 破棄済みタブ

スタッフ用記録入力ページ（`/staff/input/meal`）の「破棄済み」タブには以下が表示されます：

- **consumptionLog.handling === 'discarded'** の品物（残り対応で破棄）
- **status === 'discarded'** の品物（廃棄指示フローで廃棄）
...
| 日付 | バージョン | 変更内容 | 担当 |
|------|------------|----------|------|
| 2026-01-02 | 1.2.0 | 廃棄指示フロー（Phase 49）を追加 | - |

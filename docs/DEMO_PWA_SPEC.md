...
### 4.2 HOME画面（タブUI + テーブルビュー + 年月フィルタ）
...
|  [◀] [2025年] [▶]                               |  ← 年ページネーション
+--------------------------------------------------+
|  [全月] [1月] [2月] ... [11月] [12月]  →         |  ← 月フィルタ（横スクロール）
+--------------------------------------------------+
|  [食事 1090] [水分 1626] [排便 1985] [...]  →    |  ← シートタブ
+--------------------------------------------------+
| [🔍 検索...]                    [↕️ ソート ▼]    |
+--------------------------------------------------+
|                                                  |
...
| | 日時         | スタッフ | 入居者   | 体温   | → |
| +----------------------------------------------+ |
| | 12/13 09:00  | 田中花子 | 山田太郎 | 36.5  | → |
| | 12/13 09:15  | 佐藤一郎 | 鈴木花子 | 36.8  | → |
| +----------------------------------------------+ |
|                                                  |
|  ─── 11月 (312件) ───                           |
| +----------------------------------------------+ |
...
| 動作 | ◀▶ボタンで年を切り替え |
| デフォルト | 現在年（2025年） |
| 利用可能年 | データが存在する年のみ（2024年、2025年） |
| 件数表示 | 選択中シートの当該年の件数をバッジ表示 |

#### 月フィルタ
| 項目 | 仕様 |
|------|------|
| 表示形式 | 横スクロール可能なタブ `[全月] [1月] [2月] ... [12月]` |
| 動作 | タップで該当月のみ表示、「全月」で全件表示 |
| デフォルト | 全月（全件表示） |
| 件数表示 | 各月の件数をバッジ表示 |
| アクティブ状態 | 選択中の月をハイライト |
...
**改行表示の仕様**:
- スプレッドシートのセル内改行（`\n`）はHTMLで正しく改行表示
- `whitespace-pre-line` CSSプロパティを使用
- 例: 「【ケアに関すること】\n\n【ACPiece】」→ 改行で分割表示
...
### 10.1 現状の問題点（プロWebデザイナーによるレビュー）

| 問題 | 詳細 |
|------|------|
| 視覚的階層の欠如 | 要素間の区別がつきにくい、視線誘導不足 |
| カラーパレット単調 | 青一色で単調、アクセントカラーなし |
| タイポグラフィ | 見出し・本文の区別が曖昧、ウェイト活用不足 |
| レイアウト | 余白不足、グルーピング不明確、モバイル最適化不足 |
...
| 優先度 | 項目 |
|--------|------|
| 高 | カラーパレット適用、ヘッダーデザイン改善 |
| 高 | 月フィルタ・シートタブのスタイル改善 |
| 中 | シート別アイコン追加 |
| 中 | モバイル向けボトムナビゲーション |
| 低 | ダークモード対応 |
...
| [DESIGN_GUIDELINES.md](./DESIGN_GUIDELINES.md) | デザインガイドライン（Phase 4.5） |

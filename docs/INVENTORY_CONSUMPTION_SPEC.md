---
status: working
scope: feature
owner: core-team
last_reviewed: 2025-12-20
---

# 在庫・消費追跡システム 詳細設計書

> **最終更新**: 2025年12月16日
>
> 本ドキュメントは、品物の在庫管理と消費追跡システムの詳細設計を定義します。
> 家族とスタッフ間のデータ交差を実現し、詳細な在庫・摂食傾向分析を可能にします。

---

## 1. 概要

### 1.1 背景・目的

従来の品物管理では以下の課題がありました：

| 課題 | 詳細 |
|------|------|
| 在庫追跡の粒度が粗い | 1回の提供・摂食しか記録できない |
| 消費履歴が残らない | 複数回に分けた提供履歴が追跡できない |
| 部分消費が表現できない | 「4房中1房の半分を食べた」が記録できない |
| 食品別統計がない | 食品ごとの好み傾向が分析できない |

### 1.2 設計目標

1. **詳細な在庫追跡**: 小数点単位での残量管理
2. **消費履歴の蓄積**: 全ての提供・摂食を時系列で記録
3. **双方向の情報共有**: 家族⇄スタッフ間でリアルタイムに状況を共有
4. **食品傾向分析**: 食品別・カテゴリ別の摂食傾向を可視化

### 1.3 設計原則

1. **3層モデル**: FoodMaster / CareItem / ConsumptionLog の分離
2. **イミュータブル履歴**: 消費ログは追記のみ、編集・削除は管理者限定
3. **リアルタイム集計**: `currentQuantity` は消費ログから自動計算
4. **後方互換性**: 既存の CareItem データは移行可能

---

## 2. データモデル

### 2.1 3層モデル概要

```
┌─────────────────────────────────────────────────────────────┐
│              FoodMaster（食品マスタ）                        │
│  正規化された食品情報 + 統計データ                           │
│  例: "バナナ" - 平均賞味期限7日、平均摂食率85%              │
└─────────────────────────────────────────────────────────────┘
                              ↓ 参照（任意）
┌─────────────────────────────────────────────────────────────┐
│                CareItem（品物/在庫）                         │
│  家族が登録する在庫単位                                      │
│  例: "12/16送付のバナナ4房" - 現在残量2.5房                 │
└─────────────────────────────────────────────────────────────┘
                              ↓ 1:N
┌─────────────────────────────────────────────────────────────┐
│             ConsumptionLog（消費ログ）                       │
│  スタッフが記録する提供・摂食の履歴                          │
│  例: "12/17に1房提供、0.5房消費"                            │
└─────────────────────────────────────────────────────────────┘
```

### 2.2 FoodMaster（食品マスタ）

食品の正規化情報と統計を管理します。AI提案（aiSuggest）と連携し、**学習するキャッシュ**として機能します。

#### 2.2.1 AI連携フロー（自動蓄積）

```
家族が品物名を入力（例: "キウイ"）
         ↓
    aiSuggest API 呼び出し
         ↓
┌────────────────────────────────────────────────────────────────┐
│ Step 1: FoodMaster検索                                         │
│ - 名前・別名で完全一致 → 部分一致の順でマッチング               │
│ - 見つかった場合: source="food_master" でキャッシュ返却         │
│ - 見つからない場合: Step 2 へ                                   │
├────────────────────────────────────────────────────────────────┤
│ Step 2: Gemini API で生成                                      │
│ - 賞味期限、保存方法、提供方法を AI が提案                      │
│ - source="ai" で返却                                           │
├────────────────────────────────────────────────────────────────┤
│ Step 3: FoodMaster に自動保存（推奨設定）                       │
│ - saveToFoodMaster=true の場合、AI生成結果を自動保存            │
│ - 次回同じ品物は Step 1 でキャッシュヒット                      │
└────────────────────────────────────────────────────────────────┘
```

#### 2.2.2 自動蓄積のメリット

| メリット | 説明 |
|----------|------|
| **APIコスト削減** | 同じ品物は2回目以降Gemini APIを呼ばない |
| **応答速度向上** | Firestoreからの即座の返却（~100ms vs ~2s） |
| **施設固有データ構築** | 時間とともに利用パターンに特化した食品マスタが自動構築 |
| **統計データ蓄積** | 摂食率・好み度が蓄積され、将来の分析・提案に活用 |

#### 2.2.3 データソース（source フィールド）

| source | 説明 | 信頼度 |
|--------|------|--------|
| `food_master` | FoodMasterキャッシュから返却 | 高（実績ベース） |
| `ai` | Gemini APIで新規生成 | 中（AI推定） |
| `default` | フォールバック（AI障害時） | 低（汎用値） |

#### 2.2.4 FoodMasterデータ構造

```typescript
// Firestore: food_masters/{foodId}
interface FoodMaster {
  // === 識別情報 ===
  id: string;                          // ドキュメントID

  // === 基本情報 ===
  name: string;                        // 正規化された食品名（例: "バナナ"）
  aliases: string[];                   // 別名（"ばなな", "banana", "バナナ（フィリピン産）"）
  category: ItemCategory;              // カテゴリ

  // === デフォルト値（AI提案のベース）===
  defaultUnit: string;                 // デフォルト単位（房、個、本、袋）
  defaultExpirationDays: number;       // 平均賞味期限（日）
  defaultStorageMethod: StorageMethod; // 推奨保存方法
  defaultServingMethods: ServingMethod[]; // 推奨提供方法

  // === 注意事項 ===
  careNotes?: string;                  // ケア時の注意点（誤嚥リスク等）
  allergyInfo?: string;                // アレルギー情報

  // === 統計データ（定期バッチで更新）===
  stats: {
    totalServed: number;               // 累計提供回数
    totalConsumed: number;             // 累計消費量
    avgConsumptionRate: number;        // 平均摂食率（0-100）
    preferenceScore: number;           // 好み度スコア（0-100）
    wasteRate: number;                 // 廃棄率（0-100）
    lastUpdated: Timestamp;
  };

  // === メタ情報 ===
  isActive: boolean;                   // 有効フラグ
  createdAt: Timestamp;
  updatedAt: Timestamp;
}
```

### 2.3 CareItem（品物/在庫）- 拡張版

家族が登録する在庫単位。消費ログと連携して残量をリアルタイム更新。

```typescript
// Firestore: care_items/{itemId}
interface CareItem {
  // === 識別情報 ===
  id: string;                          // ドキュメントID（UUID）
  residentId: string;                  // 入居者ID
  userId: string;                      // 登録した家族のID

  // === 食品マスタ参照 ===
  foodMasterId?: string;               // 食品マスタID（任意、統計連携用）

  // === 品物基本情報（家族が入力）===
  itemName: string;                    // 品物名（必須）
  category: ItemCategory;              // カテゴリ（必須）
  sentDate: string;                    // 送付日 (YYYY-MM-DD)（必須）

  // === 在庫情報（数量管理の核心）===
  initialQuantity: number;             // 初期数量（例: 4）
  unit: string;                        // 単位（例: "房"）
  currentQuantity: number;             // 現在の残量（例: 2.5）★自動更新

  // === 期限・保存 ===
  expirationDate?: string;             // 賞味期限 (YYYY-MM-DD)
  storageMethod?: StorageMethod;       // 保存方法

  // === 提供希望（家族→スタッフ）===
  servingMethod: ServingMethod;        // 提供方法（必須）
  servingMethodDetail?: string;        // 提供方法詳細
  preferredServingSchedule?: string;   // 提供希望スケジュール（例: "毎日1個ずつ"）
  plannedServeDate?: string;           // 提供予定日
  noteToStaff?: string;                // スタッフへの申し送り

  // === ステータス ===
  status: ItemStatus;                  // pending | in_progress | consumed | expired | discarded

  // === 集計キャッシュ（パフォーマンス用）===
  consumptionSummary: {
    totalServed: number;               // 累計提供回数
    totalServedQuantity: number;       // 累計提供量
    totalConsumedQuantity: number;     // 累計消費量
    avgConsumptionRate: number;        // 平均摂食率
    lastServedDate?: string;           // 最終提供日
    lastServedBy?: string;             // 最終提供者
  };

  // === メタ情報 ===
  createdAt: Timestamp;
  updatedAt: Timestamp;
}

// ステータス定義（拡張）
type ItemStatus =
  | 'pending'      // 未提供（登録済み、まだ一度も提供していない）
  | 'in_progress'  // 提供中（一部提供済み、残量あり）★新規追加
  | 'consumed'     // 消費完了（残量ゼロ）
  | 'expired'      // 期限切れ
  | 'discarded';   // 廃棄
```

### 2.4 ConsumptionLog（消費ログ）- 新規

スタッフが記録する提供・摂食の履歴。品物ごとにサブコレクションとして管理。

```typescript
// Firestore: care_items/{itemId}/consumption_logs/{logId}
interface ConsumptionLog {
  // === 識別情報 ===
  id: string;                          // ドキュメントID（UUID）
  itemId: string;                      // 親の品物ID

  // === 提供情報 ===
  servedDate: string;                  // 提供日 (YYYY-MM-DD)
  servedTime?: string;                 // 提供時刻 (HH:mm)（任意）
  mealTime?: MealTime;                 // 食事時間帯（朝食/昼食/夕食/間食）
  servedQuantity: number;              // 提供数量（例: 1）
  servedBy: string;                    // 提供スタッフ名

  // === 摂食情報 ===
  consumedQuantity: number;            // 実際に食べた量（例: 0.5）
  consumptionRate: number;             // 摂食率（0-100%）
  consumptionStatus: ConsumptionStatus; // 摂食状況（full/most/half/little/none）

  // === 残量情報 ===
  quantityBefore: number;              // 提供前の残量
  quantityAfter: number;               // 提供後の残量（= quantityBefore - consumedQuantity）

  // === 特記事項・申し送り ===
  consumptionNote?: string;            // 摂食時の特記事項
  noteToFamily?: string;               // 家族への申し送り

  // === 家族指示対応（間食記録連携用）===
  followedInstruction?: boolean;       // 家族指示に従ったか
  instructionNote?: string;            // 指示対応メモ
  linkedMealRecordId?: string;         // 食事記録の投稿ID（Sheet Bとの紐づけ）
  sourceType?: 'meal_form' | 'item_detail' | 'task'; // 記録元

  // === メタ情報 ===
  recordedBy: string;                  // 記録者
  recordedAt: Timestamp;               // 記録日時
  updatedAt?: Timestamp;               // 更新日時（修正時）
  updatedBy?: string;                  // 更新者（修正時）
}

// 食事時間帯
type MealTime = 'breakfast' | 'lunch' | 'dinner' | 'snack';

const MEAL_TIMES: { value: MealTime; label: string }[] = [
  { value: 'breakfast', label: '朝食' },
  { value: 'lunch', label: '昼食' },
  { value: 'dinner', label: '夕食' },
  { value: 'snack', label: '間食' },
];
```

### 2.5 列挙型定義

```typescript
// カテゴリ（既存）
type ItemCategory =
  | 'fruit'       // 果物
  | 'snack'       // お菓子・間食
  | 'drink'       // 飲み物
  | 'dairy'       // 乳製品
  | 'prepared'    // 調理済み食品
  | 'supplement'  // 栄養補助食品
  | 'other';      // その他

// 保存方法（既存）
type StorageMethod =
  | 'room_temp'    // 常温
  | 'refrigerated' // 冷蔵
  | 'frozen';      // 冷凍

// 提供方法（既存）
type ServingMethod =
  | 'as_is'      // そのまま
  | 'cut'        // カット
  | 'peeled'     // 皮むき
  | 'heated'     // 温める
  | 'cooled'     // 冷やす
  | 'blended'    // ミキサー
  | 'other';     // その他

// 摂食状況（既存）
type ConsumptionStatus =
  | 'full'     // 完食 (100%)
  | 'most'     // ほぼ完食 (80%以上)
  | 'half'     // 半分程度 (50%程度)
  | 'little'   // 少量 (30%以下)
  | 'none';    // 食べなかった (0%)

// ステータス（拡張）
type ItemStatus =
  | 'pending'      // 未提供
  | 'in_progress'  // 提供中（一部消費）★新規
  | 'consumed'     // 消費完了
  | 'expired'      // 期限切れ
  | 'discarded';   // 廃棄
```

---

## 3. データフロー

### 3.1 品物登録フロー（家族）

```
家族が品物登録
      ↓
┌─────────────────────────────────────────┐
│ 1. CareItem ドキュメント作成            │
│    - initialQuantity = 入力値           │
│    - currentQuantity = initialQuantity  │
│    - status = 'pending'                 │
│    - consumptionSummary = 初期値        │
└─────────────────────────────────────────┘
      ↓
┌─────────────────────────────────────────┐
│ 2. FoodMaster 検索・紐付け（任意）      │
│    - itemName で食品マスタを検索        │
│    - マッチすれば foodMasterId を設定   │
└─────────────────────────────────────────┘
      ↓
┌─────────────────────────────────────────┐
│ 3. タスク自動生成（既存機能）           │
│    - 賞味期限警告タスク                 │
│    - 提供リマインダータスク             │
└─────────────────────────────────────────┘
```

### 3.2 消費記録フロー（スタッフ）

```
スタッフが提供・摂食を記録
      ↓
┌─────────────────────────────────────────┐
│ 1. バリデーション                       │
│    - servedQuantity <= currentQuantity  │
│    - consumedQuantity <= servedQuantity │
└─────────────────────────────────────────┘
      ↓
┌─────────────────────────────────────────┐
│ 2. ConsumptionLog ドキュメント作成      │
│    - quantityBefore = currentQuantity   │
│    - quantityAfter = before - consumed  │
└─────────────────────────────────────────┘
      ↓
┌─────────────────────────────────────────┐
│ 3. CareItem 更新（トランザクション）    │
│    - currentQuantity = quantityAfter    │
│    - status = 判定ロジック              │
│    - consumptionSummary = 再計算        │
└─────────────────────────────────────────┘
      ↓
┌─────────────────────────────────────────┐
│ 4. FoodMaster 統計更新（非同期）        │
│    - 定期バッチで集計                   │
└─────────────────────────────────────────┘
```

### 3.3 ステータス判定ロジック

```typescript
function determineItemStatus(item: CareItem): ItemStatus {
  const today = new Date().toISOString().split('T')[0];

  // 廃棄済みはそのまま
  if (item.status === 'discarded') {
    return 'discarded';
  }

  // 期限切れチェック
  if (item.expirationDate && item.expirationDate < today) {
    return 'expired';
  }

  // 残量ゼロ = 消費完了
  if (item.currentQuantity <= 0) {
    return 'consumed';
  }

  // 一度でも提供していれば in_progress
  if (item.consumptionSummary.totalServed > 0) {
    return 'in_progress';
  }

  // まだ一度も提供していない
  return 'pending';
}
```

### 3.4 残量計算ロジック

```typescript
// 消費ログから残量を計算（整合性チェック用）
async function calculateCurrentQuantity(itemId: string): Promise<number> {
  const item = await getDoc(doc(db, 'care_items', itemId));
  const logs = await getDocs(
    collection(db, 'care_items', itemId, 'consumption_logs')
  );

  const totalConsumed = logs.docs.reduce(
    (sum, log) => sum + log.data().consumedQuantity,
    0
  );

  return item.data().initialQuantity - totalConsumed;
}

// 消費記録時の残量更新（トランザクション）
async function recordConsumption(
  itemId: string,
  input: ConsumptionLogInput
): Promise<void> {
  await runTransaction(db, async (transaction) => {
    const itemRef = doc(db, 'care_items', itemId);
    const item = await transaction.get(itemRef);

    if (!item.exists()) {
      throw new Error('Item not found');
    }

    const currentQuantity = item.data().currentQuantity;

    // バリデーション
    if (input.servedQuantity > currentQuantity) {
      throw new Error('提供数量が残量を超えています');
    }

    if (input.consumedQuantity > input.servedQuantity) {
      throw new Error('消費数量が提供数量を超えています');
    }

    // 新しい残量を計算
    // 注意: 残した分は廃棄扱い（戻さない）
    const newQuantity = currentQuantity - input.consumedQuantity;
    // または: 残した分も減らす場合
    // const newQuantity = currentQuantity - input.servedQuantity;

    // 消費ログを作成
    const logRef = doc(collection(db, 'care_items', itemId, 'consumption_logs'));
    transaction.set(logRef, {
      ...input,
      id: logRef.id,
      itemId,
      quantityBefore: currentQuantity,
      quantityAfter: newQuantity,
      recordedAt: serverTimestamp(),
    });

    // CareItem を更新
    transaction.update(itemRef, {
      currentQuantity: newQuantity,
      status: newQuantity <= 0 ? 'consumed' : 'in_progress',
      'consumptionSummary.totalServed': increment(1),
      'consumptionSummary.totalServedQuantity': increment(input.servedQuantity),
      'consumptionSummary.totalConsumedQuantity': increment(input.consumedQuantity),
      'consumptionSummary.lastServedDate': input.servedDate,
      'consumptionSummary.lastServedBy': input.servedBy,
      updatedAt: serverTimestamp(),
    });
  });
}
```

---

## 4. API仕様

### 4.1 消費ログ記録 API

```
POST /recordConsumption
```

**リクエスト**:
```typescript
interface RecordConsumptionRequest {
  itemId: string;
  servedDate: string;              // YYYY-MM-DD
  servedTime?: string;             // HH:mm
  mealTime?: MealTime;
  servedQuantity: number;
  servedBy: string;
  consumedQuantity: number;
  consumptionStatus: ConsumptionStatus;
  consumptionNote?: string;
  noteToFamily?: string;
  recordedBy: string;
}
```

**レスポンス**:
```typescript
interface RecordConsumptionResponse {
  success: boolean;
  data?: {
    logId: string;
    itemId: string;
    currentQuantity: number;       // 更新後の残量
    status: ItemStatus;            // 更新後のステータス
  };
  error?: string;
}
```

**権限**: スタッフのみ

### 4.2 消費ログ一覧取得 API

```
GET /getConsumptionLogs
```

**リクエスト（クエリパラメータ）**:
```typescript
interface GetConsumptionLogsRequest {
  itemId: string;                  // 必須
  startDate?: string;              // 開始日
  endDate?: string;                // 終了日
  limit?: number;                  // 取得件数（デフォルト: 50）
}
```

**レスポンス**:
```typescript
interface GetConsumptionLogsResponse {
  success: boolean;
  data?: {
    logs: ConsumptionLog[];
    total: number;
  };
  error?: string;
}
```

**権限**: 全ロール（閲覧のみ）

### 4.3 在庫サマリー取得 API

```
GET /getInventorySummary
```

**リクエスト（クエリパラメータ）**:
```typescript
interface GetInventorySummaryRequest {
  residentId?: string;
  status?: ItemStatus | ItemStatus[];
  includeExpiringSoon?: boolean;   // 期限3日以内のみ
}
```

**レスポンス**:
```typescript
interface InventorySummary {
  itemId: string;
  itemName: string;
  category: ItemCategory;

  // 在庫状況
  initialQuantity: number;
  currentQuantity: number;
  unit: string;
  consumedQuantity: number;
  consumptionPercentage: number;

  // 期限情報
  expirationDate?: string;
  daysUntilExpiration?: number;
  isExpiringSoon: boolean;
  isExpired: boolean;

  // 摂食傾向
  avgConsumptionRate: number;
  totalServings: number;

  // ステータス
  status: ItemStatus;

  // 最新の申し送り
  latestNoteToFamily?: string;
  latestNoteDate?: string;
}

interface GetInventorySummaryResponse {
  success: boolean;
  data?: {
    items: InventorySummary[];
    totals: {
      totalItems: number;
      pendingCount: number;
      inProgressCount: number;
      consumedCount: number;
      expiredCount: number;
      expiringSoonCount: number;
    };
  };
  error?: string;
}
```

### 4.4 食品統計取得 API

```
GET /getFoodStats
```

**レスポンス**:
```typescript
interface FoodStatsResponse {
  success: boolean;
  data?: {
    // 食品別ランキング
    mostPreferred: {
      foodName: string;
      avgConsumptionRate: number;
      totalServings: number;
    }[];

    leastPreferred: {
      foodName: string;
      avgConsumptionRate: number;
      wastedQuantity: number;
    }[];

    // カテゴリ別傾向
    categoryStats: {
      category: ItemCategory;
      avgConsumptionRate: number;
      totalItems: number;
    }[];
  };
  error?: string;
}
```

---

## 5. UI設計

### 5.1 消費記録モーダル（スタッフ用）- 拡張版

```
┌─────────────────────────────────────────────┐
│ 提供・摂食を記録                     [×]    │
├─────────────────────────────────────────────┤
│                                             │
│ 品物: バナナ                                │
│ 残量: 3.5房 / 4房                           │
│                                             │
│ ── 提供情報 ───────────────────────────────  │
│                                             │
│ 提供日 *              時間帯                │
│ ┌───────────────┐    ┌─────────────────┐   │
│ │ 2025/12/17   │    │ ▼ 間食         │   │
│ └───────────────┘    └─────────────────┘   │
│                                             │
│ 提供数量 *                                  │
│ ┌───────┐                                   │
│ │  1    │ 房  （残り 3.5房 から）          │
│ └───────┘                                   │
│                                             │
│ ── 摂食情報 ───────────────────────────────  │
│                                             │
│ 実際に食べた量                              │
│ ┌───────┐                                   │
│ │  0.5  │ 房  （提供した 1房 のうち）      │
│ └───────┘                                   │
│ ※ 残り0.5房は廃棄扱いになります            │
│                                             │
│ 摂食状況                                    │
│ ○ 完食 (100%)                              │
│ ● 半分程度 (50%)  ← 自動選択               │
│ ○ 少量 (30%以下)                           │
│ ○ 食べなかった                              │
│                                             │
│ 特記事項                                    │
│ ┌─────────────────────────────────────────┐ │
│ │ 皮が硬かったようです                    │ │
│ └─────────────────────────────────────────┘ │
│                                             │
│ ご家族への申し送り                          │
│ ┌─────────────────────────────────────────┐ │
│ │ 半分召し上がりました。次回は柔らかめの  │ │
│ │ バナナをお願いします。                  │ │
│ └─────────────────────────────────────────┘ │
│                                             │
│      [キャンセル]    [記録する]             │
│                                             │
│ 記録後の残量: 3.0房                         │
│                                             │
└─────────────────────────────────────────────┘
```

### 5.2 品物タイムライン（共有ビュー）

```
┌─────────────────────────────────────────────┐
│ ← バナナ                         [編集]     │
├─────────────────────────────────────────────┤
│                                             │
│ ┌─ 在庫状況 ───────────────────────────────┐ │
│ │ ████████░░░░░░░░  2.5房 / 4房 (37.5%)   │ │
│ │ 平均摂食率: 75%                          │ │
│ │ 賞味期限: 12/20 (あと3日)                │ │
│ └─────────────────────────────────────────┘ │
│                                             │
│ ── タイムライン ─────────────────────────── │
│                                             │
│ 📦 12/16 10:30                             │
│    家族が登録                               │
│    「バナナ4房を送りました」                │
│    スタッフへ: 毎日1房ずつ提供してください  │
│                                             │
│ 🍽️ 12/17 15:00 【間食】                    │
│    山田さんが提供: 1房                      │
│    摂食: 0.5房 (50%)                        │
│    → 残り 3.5房                             │
│    家族へ: 半分召し上がりました             │
│                                             │
│ 🍽️ 12/18 15:00 【間食】                    │
│    佐藤さんが提供: 1房                      │
│    摂食: 1房 (100%) 完食！                  │
│    → 残り 2.5房                             │
│    家族へ: 今日は完食されました！           │
│                                             │
│ ─────────────────────────────────────────── │
│ 【スタッフ用】                              │
│        [提供・摂食を記録する]               │
│                                             │
└─────────────────────────────────────────────┘
```

### 5.3 在庫サマリーカード（家族ホーム用）

```
┌─────────────────────────────────────────────┐
│ 📦 送った品物                    [もっと見る] │
├─────────────────────────────────────────────┤
│                                             │
│ ┌─────────────────────────────────────────┐ │
│ │ 🍌 バナナ              提供中 🔵        │ │
│ │ ████████░░░░  2.5/4房  期限:12/20       │ │
│ │ 「半分召し上がりました」                │ │
│ └─────────────────────────────────────────┘ │
│                                             │
│ ┌─────────────────────────────────────────┐ │
│ │ 🍊 みかん              未提供 🟡        │ │
│ │ ████████████  5/5個    期限:12/22       │ │
│ └─────────────────────────────────────────┘ │
│                                             │
│ ┌─────────────────────────────────────────┐ │
│ │ 🍮 プリン              消費済 🟢        │ │
│ │                        摂食率:100%      │ │
│ │ 「おいしそうに召し上がっていました」    │ │
│ └─────────────────────────────────────────┘ │
│                                             │
└─────────────────────────────────────────────┘
```

---

## 6. 実装ファイル構成

### 6.1 バックエンド

```
functions/src/
├── functions/
│   ├── careItems.ts              # 品物CRUD API（既存、拡張）
│   ├── consumptionLogs.ts        # 消費ログAPI（新規）
│   └── foodMasters.ts            # 食品マスタAPI（新規、将来）
├── types/
│   ├── careItem.ts               # 型定義（拡張）
│   ├── consumptionLog.ts         # 消費ログ型定義（新規）
│   └── foodMaster.ts             # 食品マスタ型定義（新規）
└── services/
    ├── inventoryService.ts       # 在庫計算ロジック（新規）
    └── statsService.ts           # 統計計算ロジック（拡張）
```

### 6.2 フロントエンド

```
frontend/src/
├── types/
│   ├── careItem.ts               # 型定義（拡張）
│   └── consumptionLog.ts         # 消費ログ型定義（新規）
├── pages/
│   ├── family/
│   │   └── ItemTimeline.tsx      # 品物タイムライン（新規）
│   └── staff/
│       └── FamilyMessageDetail.tsx # 連絡詳細+消費記録（新規）
├── components/
│   ├── shared/
│   │   ├── InventoryBar.tsx      # 在庫バー（新規）
│   │   └── TimelineView.tsx      # タイムライン表示（新規）
│   └── staff/
│       └── ConsumptionRecordModal.tsx # 消費記録モーダル（拡張）
├── hooks/
│   ├── useConsumptionLogs.ts     # 消費ログ取得（新規）
│   └── useInventorySummary.ts    # 在庫サマリー取得（新規）
└── api/
    └── consumptionLogs.ts        # API呼び出し関数（新規）
```

---

## 7. マイグレーション計画

### 7.1 既存データの移行

既存の `CareItem` データは以下のように移行します：

```typescript
// マイグレーションスクリプト
async function migrateCareItems() {
  const items = await getDocs(collection(db, 'care_items'));

  for (const doc of items.docs) {
    const item = doc.data();

    // 新フィールドを追加
    await updateDoc(doc.ref, {
      // initialQuantity がなければ quantity を使用
      initialQuantity: item.initialQuantity ?? item.quantity ?? 1,
      // currentQuantity がなければ remainingQuantity を使用
      currentQuantity: item.currentQuantity ?? item.remainingQuantity ?? item.quantity ?? 1,
      // consumptionSummary を初期化
      consumptionSummary: item.consumptionSummary ?? {
        totalServed: item.actualServeDate ? 1 : 0,
        totalServedQuantity: item.servedQuantity ?? 0,
        totalConsumedQuantity: item.consumptionRate
          ? (item.servedQuantity ?? 0) * (item.consumptionRate / 100)
          : 0,
        avgConsumptionRate: item.consumptionRate ?? 0,
        lastServedDate: item.actualServeDate,
        lastServedBy: item.servedBy,
      },
    });

    // 既存の摂食記録があれば ConsumptionLog として移行
    if (item.actualServeDate && item.servedQuantity) {
      const logRef = doc(collection(db, 'care_items', doc.id, 'consumption_logs'));
      await setDoc(logRef, {
        id: logRef.id,
        itemId: doc.id,
        servedDate: item.actualServeDate,
        servedQuantity: item.servedQuantity,
        servedBy: item.servedBy ?? 'unknown',
        consumedQuantity: item.consumptionRate
          ? item.servedQuantity * (item.consumptionRate / 100)
          : item.servedQuantity,
        consumptionRate: item.consumptionRate ?? 100,
        consumptionStatus: item.consumptionStatus ?? 'full',
        quantityBefore: item.quantity ?? 1,
        quantityAfter: item.remainingQuantity ?? 0,
        consumptionNote: item.consumptionNote,
        noteToFamily: item.noteToFamily,
        recordedBy: item.recordedBy ?? 'unknown',
        recordedAt: item.updatedAt ?? serverTimestamp(),
      });
    }
  }
}
```

### 7.2 後方互換性

- 既存のAPIは維持し、新APIを追加
- フロントエンドは段階的に新UIに移行
- 旧フィールド（`quantity`, `remainingQuantity`）は読み取り専用で残す

---

## 8. 参照資料

- [ITEM_MANAGEMENT_SPEC.md](./ITEM_MANAGEMENT_SPEC.md) - 品物管理詳細設計
- [SNACK_RECORD_INTEGRATION_SPEC.md](./SNACK_RECORD_INTEGRATION_SPEC.md) - 間食記録連携設計（食事記録からの消費ログ連携）
- [VIEW_ARCHITECTURE_SPEC.md](./VIEW_ARCHITECTURE_SPEC.md) - ビュー構成設計
- [USER_ROLE_SPEC.md](./USER_ROLE_SPEC.md) - ユーザーロール・権限設計
- [STATS_DASHBOARD_SPEC.md](./STATS_DASHBOARD_SPEC.md) - 統計ダッシュボード設計

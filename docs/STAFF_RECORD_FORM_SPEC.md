...
[記録を保存] クリック
    ...
    │       └─→ photoUrl を取得
    │
    ├─2→ recordConsumptionLog API（消費ログ記録）
    │       └─→ 在庫・摂食データを記録
    │       （photoUrl は含めない - care_photos で別管理）
    │
    └─3→ submitMealRecord API（Sheet B + Webhook）
            └─→ photoUrl を含めて送信（Google Chat Webhook用）
```

> **注**: 写真メタデータは `care_photos` コレクションで管理され、
> `residentId + date + mealTime` で消費ログと関連付けられます。
> `submitMealRecord` に `photoUrl` を渡すことで、Google Chat に写真プレビューが表示されます。
...
| ID | テストケース | 期待結果 | 状態 |
|----|-------------|----------|------|
| STAFF-009 | ダイアログ内に写真アップロードが表示される | 写真アップロードエリアが存在 | ✅ |
| STAFF-070 | ダイアログ内に写真追加ボタンが表示される | 写真ボタン/ラベルが表示 | ✅ |
| STAFF-071 | 写真ファイル入力が存在する | `input[type="file"][accept*="image"]` が存在 | ✅ |
| STAFF-072 | 写真セクションのラベルが表示される | 「写真」ラベルが表示 | ✅ |

**将来追加検討**（実APIテストが必要）:
| ID | テストケース | 備考 |
|----|-------------|------|
| STAFF-073 | 写真選択後にプレビューが表示される | ファイル選択モック必要 |
| STAFF-074 | 写真を削除できる | プレビュー状態のテスト |
| STAFF-075 | 写真付きで記録を保存できる | API統合テスト |
...
|----------|------|------|
| 15.9.1 | 設計ドキュメント更新 | ✅ |
| 15.9.2 | E2Eテスト追加（TDD） | ✅ STAFF-009, STAFF-070〜072 |
| 15.9.3 | StaffRecordDialog.tsx 写真UI実装 | ✅ |
| 15.9.4 | アップロードロジック実装 | ✅ |
| 15.9.5 | ローカルテスト実行 | ✅ 34 passed |
| 15.9.6 | コミット・Push | ✅ `2bd7bff`, `1ee4518` |
| 15.9.7 | 本番E2Eテスト | ✅ 34 passed |

**コミット履歴**:
- `2bd7bff`: Phase 15.9 写真アップロードUI実装
- `1ee4518`: photoUrl → submitMealRecord連携修正（設計準拠）

...
- 現在 216件 のテストケースがパス（Phase 15.7で4件追加）
...
| Phase 15.7 | 残り対応による在庫・統計分離（STAFF-050〜053） |

#### Phase 15.7 E2Eテストケース（2025-12-20 追加）

| ID | テストケース | 期待結果 | 状態 |
|----|-------------|----------|------|
| STAFF-050 | 「破棄した」選択時の在庫計算 | 提供量全てが在庫から引かれ、廃棄量表示 | ✅ |
| STAFF-051 | 「保存した」選択時の在庫計算 | 食べた分のみ在庫から引かれる | ✅ |
| STAFF-052 | 残り対応切替での残量プレビュー更新 | 即座にプレビューが再計算される | ✅ |
| STAFF-053 | 完食（100%）時の残り対応不要 | 残り対応フィールドが非表示 | ✅ |

**実行コマンド**:
```bash
BASE_URL=https://facility-care-input-form.web.app npx playwright test e2e/staff-record-form.spec.ts --grep "STAFF-05"
```
...
| 日付 | 変更内容 |
|------|----------|
| 2025-12-20 | Phase 15.7 E2Eテストケース（STAFF-050〜053）追加、テスト件数216件に更新 |

# タスク管理機能 詳細設計書

> **最終更新**: 2025年12月16日
>
> 本ドキュメントは、品物管理・ケア指示に関連するタスクの自動生成・管理機能の詳細設計を定義します。

---

## 1. 概要

### 1.1 目的

介護施設のスタッフが「今日何をすべきか」を明確に把握し、漏れなく対応できるようにします：

1. **賞味期限管理**: 期限が近い品物の提供を促す
2. **提供リマインダー**: 家族の希望提供日に合わせて通知
3. **ケア指示確認**: 家族からの指示を確認・対応
4. **進捗可視化**: タスクの完了状況を家族・スタッフで共有

### 1.2 タスクの種類

| タスクタイプ | 生成元 | 説明 |
|-------------|--------|------|
| `expiration_warning` | 品物 | 賞味期限が近い品物の警告 |
| `serve_reminder` | 品物 | 提供予定日のリマインダー |
| `restock_alert` | 品物 | 在庫切れアラート |
| `care_instruction` | ケア指示 | 新しいケア指示の確認 |
| `custom` | スタッフ/管理者 | 手動作成のカスタムタスク |

### 1.3 タスクライフサイクル

```
┌─────────────────────────────────────────────────────────────┐
│                    タスクライフサイクル                       │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│    [自動生成]          [確認]           [完了]              │
│        │                 │                │                │
│        ▼                 ▼                ▼                │
│    ┌───────┐         ┌───────┐        ┌───────┐           │
│    │pending│ ──────> │in_prog│ ─────> │complet│           │
│    └───────┘         └───────┘        └───────┘           │
│        │                 │                                 │
│        │                 │            ┌───────┐           │
│        └─────────────────┴──────────> │cancell│           │
│                                       └───────┘           │
│                     [キャンセル]                           │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## 2. データモデル

### 2.1 Task（タスク）

```typescript
// Firestore: tasks/{taskId}
interface Task {
  // === 識別情報 ===
  id: string;                    // ドキュメントID（自動生成 UUID）
  residentId: string;            // 入居者ID

  // === タスク内容 ===
  title: string;                 // タスクタイトル（必須）
  description?: string;          // 詳細説明
  taskType: TaskType;            // タスク種別（必須）

  // === 関連データ ===
  relatedItemId?: string;        // 関連する品物ID（care_items）
  relatedInstructionId?: string; // 関連するケア指示ID（care_instructions）

  // === スケジュール ===
  dueDate: string;               // 期日 (YYYY-MM-DD)（必須）
  dueTime?: string;              // 時刻 (HH:mm)

  // === ステータス・優先度 ===
  status: TaskStatus;            // ステータス（必須）
  priority: TaskPriority;        // 優先度（必須）

  // === 担当・実行 ===
  assignee?: string;             // 担当者（スタッフ名）
  completedBy?: string;          // 完了者（スタッフ名）
  completionNote?: string;       // 完了時のメモ

  // === 通知 ===
  notificationSent: boolean;     // 通知送信済みフラグ
  notificationScheduledAt?: Timestamp; // 通知予定日時

  // === 生成情報 ===
  createdBy?: string;            // 作成者（'system' or スタッフ名）
  isAutoGenerated: boolean;      // 自動生成フラグ

  // === メタ情報 ===
  createdAt: Timestamp;          // 作成日時
  updatedAt: Timestamp;          // 更新日時
  completedAt?: Timestamp;       // 完了日時
}
```

### 2.2 列挙型定義

```typescript
// タスク種別
type TaskType =
  | 'expiration_warning'  // 賞味期限警告
  | 'serve_reminder'      // 提供リマインダー
  | 'restock_alert'       // 在庫切れアラート
  | 'care_instruction'    // ケア指示確認
  | 'custom';             // カスタムタスク

const TASK_TYPES: { value: TaskType; label: string; icon: string }[] = [
  { value: 'expiration_warning', label: '賞味期限', icon: '⏰' },
  { value: 'serve_reminder', label: '提供予定', icon: '🍽️' },
  { value: 'restock_alert', label: '在庫切れ', icon: '📦' },
  { value: 'care_instruction', label: 'ケア指示', icon: '📋' },
  { value: 'custom', label: 'その他', icon: '📝' },
];

// タスクステータス
type TaskStatus =
  | 'pending'      // 未着手
  | 'in_progress'  // 対応中
  | 'completed'    // 完了
  | 'cancelled';   // キャンセル

const TASK_STATUSES: { value: TaskStatus; label: string; color: string }[] = [
  { value: 'pending', label: '未着手', color: 'yellow' },
  { value: 'in_progress', label: '対応中', color: 'blue' },
  { value: 'completed', label: '完了', color: 'green' },
  { value: 'cancelled', label: 'キャンセル', color: 'gray' },
];

// 優先度
type TaskPriority =
  | 'low'     // 低
  | 'medium'  // 中
  | 'high'    // 高
  | 'urgent'; // 緊急

const TASK_PRIORITIES: { value: TaskPriority; label: string; color: string; order: number }[] = [
  { value: 'urgent', label: '緊急', color: 'red', order: 0 },
  { value: 'high', label: '高', color: 'orange', order: 1 },
  { value: 'medium', label: '中', color: 'yellow', order: 2 },
  { value: 'low', label: '低', color: 'gray', order: 3 },
];
```

---

## 3. タスク自動生成ルール

### 3.1 賞味期限警告（expiration_warning）

**トリガー**: Cloud Scheduler（毎日午前6時）

**生成条件**:
```typescript
// 品物の賞味期限が3日以内で、まだ未提供のもの
const items = await firestore
  .collection('care_items')
  .where('status', '==', 'pending')
  .where('expirationDate', '<=', addDays(today, 3))
  .where('expirationDate', '>=', today)
  .get();
```

**生成タスク**:
```typescript
{
  title: `【期限注意】${item.itemName}の賞味期限が近づいています`,
  description: `賞味期限: ${item.expirationDate}（残り${daysUntil}日）`,
  taskType: 'expiration_warning',
  relatedItemId: item.id,
  dueDate: item.expirationDate,
  priority: daysUntil <= 1 ? 'urgent' : 'high',
  status: 'pending',
  isAutoGenerated: true,
  createdBy: 'system',
}
```

### 3.2 提供リマインダー（serve_reminder）

**トリガー**: Cloud Scheduler（毎日午前6時）

**生成条件**:
```typescript
// 提供予定日が今日の品物で、まだ未提供のもの
const items = await firestore
  .collection('care_items')
  .where('status', '==', 'pending')
  .where('plannedServeDate', '==', today)
  .get();
```

**生成タスク**:
```typescript
{
  title: `【提供予定】${item.itemName}を提供してください`,
  description: `提供方法: ${item.servingMethod}\n${item.servingMethodDetail || ''}`,
  taskType: 'serve_reminder',
  relatedItemId: item.id,
  dueDate: today,
  priority: 'medium',
  status: 'pending',
  isAutoGenerated: true,
  createdBy: 'system',
}
```

### 3.3 在庫切れアラート（restock_alert）

**トリガー**: 品物の残数が0になった時（リアルタイム）

**生成条件**:
```typescript
// 残数が0で、同じ品物名の在庫がない
if (item.remainingQuantity === 0) {
  const hasStock = await checkSameItemStock(item.itemName, item.residentId);
  if (!hasStock) {
    // タスク生成
  }
}
```

**生成タスク**:
```typescript
{
  title: `【在庫切れ】${item.itemName}がなくなりました`,
  description: `ご家族に補充をお願いする場合は申し送りをご記入ください`,
  taskType: 'restock_alert',
  relatedItemId: item.id,
  dueDate: today,
  priority: 'low',
  status: 'pending',
  isAutoGenerated: true,
  createdBy: 'system',
}
```

### 3.4 ケア指示確認（care_instruction）

**トリガー**: 家族がケア指示を登録した時（リアルタイム）

**生成タスク**:
```typescript
{
  title: `【新規指示】${instruction.menuName}に関する指示`,
  description: instruction.processingDetail,
  taskType: 'care_instruction',
  relatedInstructionId: instruction.id,
  dueDate: instruction.targetDate,
  priority: instruction.priority === 'critical' ? 'urgent' : 'high',
  status: 'pending',
  isAutoGenerated: true,
  createdBy: 'system',
}
```

---

## 4. API仕様

### 4.1 タスク一覧取得 API

```
GET /getTasks
```

**リクエスト（クエリパラメータ）**:
```typescript
interface GetTasksRequest {
  residentId?: string;           // 入居者でフィルタ
  status?: TaskStatus | TaskStatus[]; // ステータスでフィルタ
  taskType?: TaskType;           // タイプでフィルタ
  priority?: TaskPriority;       // 優先度でフィルタ
  dueDate?: string;              // 期日でフィルタ
  dueDateRange?: {               // 期日範囲でフィルタ
    start: string;
    end: string;
  };
  limit?: number;                // 取得件数（デフォルト: 50）
  offset?: number;               // オフセット
  sortBy?: 'dueDate' | 'priority' | 'createdAt'; // ソート項目
  sortOrder?: 'asc' | 'desc';    // ソート順
}
```

**レスポンス**:
```typescript
interface GetTasksResponse {
  success: boolean;
  data?: {
    tasks: Task[];
    total: number;
    counts: {
      pending: number;
      inProgress: number;
      completed: number;
      overdue: number;  // 期限切れ
    };
  };
  error?: string;
}
```

**権限**: 全ロール（閲覧）

### 4.2 タスク詳細取得 API

```
GET /getTask/:taskId
```

**レスポンス**:
```typescript
interface GetTaskResponse {
  success: boolean;
  data?: {
    task: Task;
    relatedItem?: CareItem;       // 関連品物
    relatedInstruction?: CareInstruction; // 関連ケア指示
  };
  error?: string;
}
```

**権限**: 全ロール（閲覧）

### 4.3 タスク更新 API

```
PUT /updateTask
```

**リクエスト**:
```typescript
interface UpdateTaskRequest {
  taskId: string;
  updates: {
    status?: TaskStatus;
    assignee?: string;
    completionNote?: string;
  };
  completedBy?: string;          // 完了時に必須
}
```

**レスポンス**:
```typescript
interface UpdateTaskResponse {
  success: boolean;
  data?: {
    taskId: string;
    status: TaskStatus;
    updatedAt: string;
  };
  error?: string;
}
```

**権限**: スタッフ（完了処理）、管理者

### 4.4 カスタムタスク作成 API

```
POST /createTask
```

**リクエスト**:
```typescript
interface CreateTaskRequest {
  residentId: string;
  title: string;
  description?: string;
  dueDate: string;
  dueTime?: string;
  priority: TaskPriority;
  assignee?: string;
}
```

**レスポンス**:
```typescript
interface CreateTaskResponse {
  success: boolean;
  data?: {
    taskId: string;
    createdAt: string;
  };
  error?: string;
}
```

**権限**: スタッフ、管理者

### 4.5 タスク削除 API

```
DELETE /deleteTask
```

**リクエスト**:
```typescript
interface DeleteTaskRequest {
  taskId: string;
}
```

**権限**: 管理者のみ

---

## 5. UI設計

### 5.1 タスクダッシュボード

**表示場所**:
- スタッフ: `/staff` ホーム画面上部
- 家族: `/family` ホーム画面上部

```
┌─────────────────────────────────────────────┐
│ 📋 今日のタスク                    全て見る │
├─────────────────────────────────────────────┤
│                                             │
│ 🔴 緊急 (2)  🟠 高 (3)  🟡 中 (5)          │
│                                             │
│ ┌─────────────────────────────────────────┐ │
│ │ 🔴 ⏰ 【期限注意】キウイの賞味期限       │ │
│ │      期限: 今日 (12/16)                 │ │
│ │      [完了にする]                        │ │
│ └─────────────────────────────────────────┘ │
│                                             │
│ ┌─────────────────────────────────────────┐ │
│ │ 🟠 📋 【新規指示】キウイに関する指示     │ │
│ │      山田様からの指示                    │ │
│ │      [詳細を見る]                        │ │
│ └─────────────────────────────────────────┘ │
│                                             │
│ ┌─────────────────────────────────────────┐ │
│ │ 🟡 🍽️ 【提供予定】プリンを提供          │ │
│ │      予定: 今日                          │ │
│ │      [完了にする]                        │ │
│ └─────────────────────────────────────────┘ │
│                                             │
└─────────────────────────────────────────────┘
```

### 5.2 タスク一覧ページ（スタッフ用）

**パス**: `/staff/tasks`

```
┌─────────────────────────────────────────────┐
│ 📋 タスク一覧                               │
├─────────────────────────────────────────────┤
│ [今日] [明日] [今週] [全て]                 │
├─────────────────────────────────────────────┤
│ [未着手] [対応中] [完了]  [キャンセル]      │
├─────────────────────────────────────────────┤
│                                             │
│ ── 緊急 (2) ────────────────────────────── │
│                                             │
│ ┌─────────────────────────────────────────┐ │
│ │ ⏰ キウイの賞味期限が近づいています      │ │
│ │    期限: 12/16  入居者: 田中様           │ │
│ │    [対応する]                            │ │
│ └─────────────────────────────────────────┘ │
│                                             │
│ ── 高 (3) ──────────────────────────────── │
│                                             │
│ ┌─────────────────────────────────────────┐ │
│ │ 📋 キウイに関する指示                    │ │
│ │    山田様より  入居者: 田中様            │ │
│ │    [確認する]                            │ │
│ └─────────────────────────────────────────┘ │
│                                             │
│ ── 中 (5) ──────────────────────────────── │
│ ...                                         │
│                                             │
├─────────────────────────────────────────────┤
│ [記録閲覧] [記録入力] [家族連絡] [統計]     │
└─────────────────────────────────────────────┘
```

### 5.3 タスク詳細モーダル

```
┌─────────────────────────────────────────────┐
│ タスク詳細                          [×]    │
├─────────────────────────────────────────────┤
│                                             │
│ 🔴 緊急                                     │
│                                             │
│ ⏰ 【期限注意】キウイの賞味期限が           │
│    近づいています                           │
│                                             │
│ ┌─ 詳細 ─────────────────────────────────┐  │
│ │ 賞味期限: 2025/12/16（今日）           │  │
│ │ 入居者: 田中様                         │  │
│ │ 残数: 3個                              │  │
│ └─────────────────────────────────────────┘  │
│                                             │
│ ┌─ 関連品物 ─────────────────────────────┐  │
│ │ 🥝 キウイ                              │  │
│ │ [品物詳細を見る]                       │  │
│ └─────────────────────────────────────────┘  │
│                                             │
│ 完了メモ                                    │
│ ┌─────────────────────────────────────────┐ │
│ │ 本日中に提供しました                    │ │
│ └─────────────────────────────────────────┘ │
│                                             │
│      [キャンセル]    [完了にする]           │
│                                             │
└─────────────────────────────────────────────┘
```

### 5.4 タスクバッジコンポーネント

```typescript
// frontend/src/components/family/TaskBadge.tsx

interface TaskBadgeProps {
  count: number;
  priority?: TaskPriority;
}

function TaskBadge({ count, priority }: TaskBadgeProps) {
  if (count === 0) return null;

  const colorClass = priority
    ? TASK_PRIORITIES.find(p => p.value === priority)?.color
    : 'red';

  return (
    <span className={`task-badge bg-${colorClass}-500`}>
      {count}
    </span>
  );
}
```

---

## 6. 通知設計

### 6.1 通知トリガー

| トリガー | 通知先 | タイミング | 内容 |
|---------|--------|-----------|------|
| 賞味期限警告 | スタッフ | 毎日9:00 | 本日期限の品物一覧 |
| 提供リマインダー | スタッフ | 毎日9:00 | 本日提供予定の品物一覧 |
| ケア指示登録 | スタッフ | リアルタイム | 新しい指示の通知 |
| タスク完了 | 家族 | リアルタイム | 指示が対応完了した通知 |

### 6.2 通知内容フォーマット

**賞味期限警告（PWA Push通知）**:
```
【期限注意】本日期限の品物があります
・キウイ（田中様）- 期限: 12/16
・プリン（山田様）- 期限: 12/16
```

**ケア指示登録（PWA Push通知）**:
```
【新規指示】山田様からケア指示が届きました
田中様のキウイに関する指示です。
内容を確認してください。
```

### 6.3 Cloud Schedulerジョブ

```typescript
// functions/src/functions/taskGenerator.ts

// 毎日午前6時に実行: タスク自動生成
export const generateDailyTasks = functions
  .region('asia-northeast1')
  .pubsub.schedule('0 6 * * *')
  .timeZone('Asia/Tokyo')
  .onRun(async (context) => {
    await generateExpirationWarnings();
    await generateServeReminders();
  });

// 毎日午前9時に実行: 通知送信
export const sendDailyTaskNotifications = functions
  .region('asia-northeast1')
  .pubsub.schedule('0 9 * * *')
  .timeZone('Asia/Tokyo')
  .onRun(async (context) => {
    await sendExpirationNotifications();
    await sendServeReminderNotifications();
  });
```

---

## 7. 実装ファイル構成

### 7.1 バックエンド

```
functions/src/
├── functions/
│   ├── tasks.ts               # タスクCRUD API
│   └── taskGenerator.ts       # タスク自動生成（Scheduler）
├── types/
│   └── task.ts                # 型定義
└── services/
    ├── taskService.ts         # ビジネスロジック
    └── taskNotificationService.ts # 通知ロジック
```

### 7.2 フロントエンド

```
frontend/src/
├── types/
│   └── task.ts                # 型定義・定数
├── pages/
│   ├── staff/
│   │   └── TaskList.tsx       # タスク一覧ページ
│   └── family/
│       └── TaskDashboard.tsx  # タスクダッシュボード
├── components/family/
│   ├── TaskCard.tsx           # タスクカード
│   ├── TaskList.tsx           # タスクリスト
│   ├── TaskBadge.tsx          # タスクバッジ
│   ├── TaskDetailModal.tsx    # タスク詳細モーダル
│   └── TaskFilters.tsx        # フィルタコンポーネント
├── hooks/
│   ├── useTasks.ts            # タスク一覧取得
│   ├── useTask.ts             # タスク詳細取得
│   ├── useTaskCounts.ts       # タスク件数取得
│   └── useTaskMutations.ts    # 更新・完了処理
└── api/
    └── tasks.ts               # API呼び出し関数
```

---

## 8. 参照資料

- [USER_ROLE_SPEC.md](./USER_ROLE_SPEC.md) - ユーザーロール・権限設計
- [ITEM_MANAGEMENT_SPEC.md](./ITEM_MANAGEMENT_SPEC.md) - 品物管理詳細設計
- [STATS_DASHBOARD_SPEC.md](./STATS_DASHBOARD_SPEC.md) - 統計ダッシュボード設計
- [FAMILY_UX_DESIGN.md](./FAMILY_UX_DESIGN.md) - 家族向けUX設計

...
| **E2Eテスト** | 328件定義・275件パス・39件スキップ（2025-12-22時点）|
...
| Phase | 内容 | 完了日 |
|-------|------|--------|
| **Phase 37** | **プリセット管理へのアクセス改善** | 12/22 |
| Phase 30.1 | 品物削除通知・記録チェック時間設定 | 12/21 |
...
- **Phase 37完了**: プリセット管理へのアクセス改善
  - 品物管理ページのヘッダーに「⭐ いつもの指示」リンクを追加
  - レスポンシブ対応: モバイルではアイコンのみ、タブレット以上でテキスト表示
  - デモ/本番両モード対応
  - E2Eテスト5件追加（preset-navigation.spec.ts）
  - 設計書: [PRESET_MANAGEMENT_SPEC.md](./PRESET_MANAGEMENT_SPEC.md) セクション5.1

### 昨日の修正 (12/21)

- **Phase 30.1完了**: 品物削除通知・記録チェック時間設定
  - 品物削除時のGoogle Chat通知追加（#品物削除🗑️）
  - 記録チェック通知時間を管理画面で設定可能に（0-23時、デフォルト16時）
  - Cloud Scheduler: 毎日16時固定 → 毎時実行＋設定時刻比較
...
- **Phase 35完了**: 同期間隔調整（15分→1時間）
  ...
  - 変更前: `*/15 * * * *`（15分毎）
  - 変更後: `0 * * * *`（毎時0分）
  - 理由: 元のGAS（スプレッドシート側）が1時間毎の自動実行のため整合性確保
  - ドキュメント更新: SYNC_STRATEGY.md, ARCHITECTURE.md

- **Phase 34完了**: 統計ダッシュボードAI分析のデモモード対応
  - `/demo/stats` の摂食傾向タブでAI分析が正常に動作するように修正
  - デモモードではAPIを呼び出さず、モックデータを表示
  - 発見事項（プリン摂取率95%、キウイ完食率向上、りんご摂取率低下）
  - 改善提案（りんご提供方法変更、黒豆頻度見直し、プリンストック確保）
  - E2Eテスト9件追加（demo-stats-ai.spec.ts）
  - 設計書: [DEMO_AI_ANALYSIS_SPEC.md](./DEMO_AI_ANALYSIS_SPEC.md)

- **Phase 33完了**: 残ったものへの処置指示
  - 家族が品物登録/編集時に「残った場合の処置」を指示可能に（破棄/保存/指定なし）
  - スタッフ記録入力時、家族指示がある場合はその選択肢のみ有効化（他は非活性）
  - 家族指示バナー表示、自動選択機能
  - **デモデータ更新**: 4品目に処置指示を追加（バナナ:保存、りんご:破棄、プリン:破棄、麦茶:保存）
  - **デモツアー更新**: 家族用・スタッフ用ツアーに処置指示機能を追加
  - E2Eテスト10件（8パス/2スキップ）- デモデータ追加によりテスト有効化
  - 設計書: [REMAINING_HANDLING_INSTRUCTION_SPEC.md](./REMAINING_HANDLING_INSTRUCTION_SPEC.md)

- **Phase 32完了**: 統計ダッシュボードの「カテゴリ別分布」を「品物別分布」に変更
  - Phase 31でカテゴリが2種類に簡素化されたため、より有用な品物別表示に変更
  - 消費割合の高い順に上位6品目を表示（消費量/初期量・割合）
  - 色分け: 緑(60%以上)→青(40%以上)→黄(20%以上)→赤(20%未満)
  - 設計書: [STATS_DASHBOARD_SPEC.md](./STATS_DASHBOARD_SPEC.md) セクション3.1
  - E2Eテスト: SCENARIO-D18更新（カテゴリ別→品物別）

- **Phase 31完了**: カテゴリ簡素化（7→2）、スタッフ記録フォームタブ固定化
  - ItemCategory: fruit/snack/drink/dairy/prepared/supplement/other → food/drink
  - 後方互換性: migrateCategory() で旧データも自動変換
  - StaffRecordDialog: タブ切替を廃止、カテゴリで自動選択（固定）
  - 食べ物 → 食事フォーム（摂食割合入力）
  - 飲み物 → 水分フォーム（cc入力）
  - 統計・デモデータ・Google Chat通知も2カテゴリに対応
  - 設計書: [ITEM_MANAGEMENT_SPEC.md](./ITEM_MANAGEMENT_SPEC.md) セクション2.2
  - 設計書: [STAFF_RECORD_FORM_SPEC.md](./STAFF_RECORD_FORM_SPEC.md) タブ固定動作
...
Phase 23-31 ████████████████████  100% (UX改善・水分記録・通知・カテゴリ簡素化)
```

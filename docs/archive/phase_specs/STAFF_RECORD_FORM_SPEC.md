...
StaffRecordDialogに**タブ式UI**を採用し、記録種別を明確に分離する。

### 13.3 タブ式UIデザイン

#### 13.3.1 タブ構成

StaffRecordDialogを「食事（間食）」と「水分」の2タブ構成に変更:
...
#### 13.3.2 デフォルトタブ選択ロジック

**カテゴリ連動でデフォルトタブを自動選択**:

| 品物カテゴリ | デフォルトタブ | 理由 |
|-------------|---------------|------|
| `drink` | 💧 水分 | 飲み物は水分記録が主目的 |
| `dairy` | 🍪 食事 | 乳製品は食事が多い（ヨーグルト等）|
| `fruit` | 🍪 食事 | 果物は食事として記録 |
| `snack` | 🍪 食事 | 菓子は間食として記録 |
| `other` | 🍪 食事 | その他は食事扱い |
...
```typescript
// デフォルトタブ決定ロジック
function getDefaultTab(category: ItemCategory): 'meal' | 'hydration' {
  return category === 'drink' ? 'hydration' : 'meal';
}
```

**~~任意選択~~**: ~~ユーザーはいつでもタブを切り替え可能。~~
~~例: 牛乳（dairy）を水分として記録したい場合は「水分」タブに切り替え。~~

> **⚠️ Phase 31 変更**: タブ切り替えは廃止されました。
> 詳細は [セクション 14: Phase 31 タブ固定化](#14-phase-31-タブ固定化) を参照。
...
**記録先**: Sheet B（実績入力）、Google Chat（#食事🍚タグ）
...
**記録先**: 水分摂取量スプレッドシート、Google Chat（#水分摂取💧タグ）

#### 13.3.5 共通項目（両タブ共通）

| 項目 | 必須 | デフォルト値 | 備考 |
|------|------|-------------|------|
| 入力者名 | * | 設定から取得 | `defaultStaffName` |
| デイサービス利用中？ | * | 設定から取得 | `defaultDayServiceUsage` |
| デイサービス名 | 条件付き | 設定から取得 | 利用中の場合のみ表示 |
| 特記事項 | - | **初期値あり** | 下記参照 |
| 重要特記事項に反映？ | * | 重要ではない | |
| 施設 | 自動 | 設定から取得 | `defaultFacility` |

**特記事項の初期値**:

食事・水分どちらのタブでも、特記事項フィールドには以下の初期値が**実際に入力された状態**で表示される。
ユーザーが削除しない限り、この内容がそのまま送信される。
...
> **注意**: 現在の実装では `placeholder`（ヒントテキスト）になっているが、
> 正しくは `defaultValue`（初期値）として設定する必要がある。Phase 29実装時に修正。

> **デフォルト設定反映**: 食事・水分どちらのタブでも、設定モーダルで登録した
> デフォルト値（入力者名、施設、デイサービス）が自動適用されます。

#### 13.3.6 水分量自動計算

飲み物カテゴリで水分タブ選択時、品物の提供数と単位から水分量を自動計算:
...
タブ選択に応じて異なるAPIを呼び出し、異なるシートに記録する:
...
[記録を保存] クリック
    ...
    │
    └─2→ submitMealRecord API
            ├─→ Sheet B（実績入力）に記録
            └─→ Google Chat Webhook（#食事🍚タグ付き）
```

**投稿ID形式**: `MEL{timestamp}` または `SNK{timestamp}`
...
[記録を保存] クリック
    │
    ├─1→ recordConsumptionLog API（在庫・消費ログ）
    │       └─→ 在庫更新、consumption_logs記録（飲み物の在庫管理）
    │
    └─2→ submitCareRecord API（recordType: 'hydration'）
            ├─→ 水分摂取量シートに記録
            └─→ Google Chat Webhook（#水分摂取💧タグ付き）
```

**投稿ID形式**: `HYD{timestamp}`

> **注意**: 水分タブ選択時は `submitMealRecord` は呼ばれません。
> 食事シートと水分シートは**別々のスプレッドシート**に記録されます。

### 13.5 Google Chat Webhook通知ルール

タブ選択に応じて異なるメッセージ形式でGoogle Chatに送信する。

#### 13.5.1 共通ルール

| ルール | 食事タブ | 水分タブ |
|--------|---------|---------|
| 重要時（`isImportant === '重要'`） | `#重要⚠️` タグ付加 | `#重要⚠️` タグ付加 |
| デイサービス利用中 | `#デイ利用中[X]` タグ付加 | `#デイ利用中[X]` タグ付加 |
| デフォルト設定反映 | 入力者名・施設・デイサービス | 入力者名・施設・デイサービス |
| 投稿ID形式 | `MEL/SNK{timestamp}` | `HYD{timestamp}` |
...
| シート名 | `フォームの回答 1` |
| GID | `238048496` |
| URL | https://docs.google.com/spreadsheets/d/1su5K9TjmzMfKc8OIK2aZYXCfuDrNeIRM0a3LUCFcct4/edit?gid=238048496 |

**列マッピング**:

| 列 | ヘッダー（原文） | フィールド | 値 |
|----|-----------------|-----------|-----|
| A | タイムスタンプ | timestamp | ISO日時 or `YYYY/MM/DD HH:mm:ss` |
| B | あなたの名前は？ | staffName | スタッフ名 |
| C | ご利用者様のお名前は？ | residentName | 入居者名 |
| D | 水分量はいくらでしたか？ | hydrationAmount | 数値（cc） |
| E | 特記事項 | note | 自由記述 |
| F | 重要特記事項集計表に反映させますか？ | isImportant | `重要` / `重要ではない` |
| G | 施設 | facility | 施設名（デフォルト設定から） |
| H | デイ利用有無 | dayServiceUsage | `利用中` / `利用中ではない` |
| I | 投稿ID | postId | `HYD{timestamp}` 形式 |
| J | どこのデイサービスですか？ | dayServiceName | デイサービス名（条件付き）|

> **注意**: このシートは食事記録（Sheet B）とは**別のスプレッドシート**です。
...
interface FormData {
  ...
  // Phase 29: タブ式UI・水分記録
  activeTab: 'meal' | 'hydration';   // 選択中のタブ
...
| ID | テストケース | 期待結果 |
|----|-------------|----------|
| STAFF-080 | 飲み物カテゴリで水分タブがデフォルト選択 | 水分タブがactive |
| STAFF-081 | 食品カテゴリで食事タブがデフォルト選択 | 食事タブがactive |
| STAFF-082 | タブ切り替えが可能 | クリックでタブ変更 |
| STAFF-083 | 水分タブで水分量入力フィールドが表示 | フィールド存在 |
| STAFF-084 | 食事タブで摂食割合入力が表示 | フィールド存在 |

#### 水分記録関連

| ID | テストケース | 期待結果 |
|----|-------------|----------|
| STAFF-085 | 飲み物で水分量が自動計算される | 提供数から計算 |
| STAFF-086 | 水分量を手動編集できる | 編集可能 |
| STAFF-087 | 特記事項に初期値が設定されている | デフォルト値あり |
| STAFF-088 | 両タブでタブUIが表示される | タブUI表示 |

#### 表示・UI関連

| ID | テストケース | 期待結果 |
|----|-------------|----------|
| STAFF-089 | 食事タブに絵文字アイコンが表示される | 🍪表示 |
| STAFF-090 | 水分タブに絵文字アイコンが表示される | 💧表示 |
| STAFF-091 | 水分タブで重要選択が可能 | ラジオボタン機能 |
| STAFF-092 | デイサービス選択が水分タブでも機能する | 選択可能 |

#### 残った分への対応（水分タブ）

| ID | テストケース | 期待結果 |
|----|-------------|----------|
| STAFF-093 | 水分タブで全量消費時は残り対応が非表示 | UI非表示 |
| STAFF-094 | 水分タブで全量未消費時に残り対応が表示される | UI表示 |
| STAFF-095 | 水分タブで残り対応未選択時はエラー | バリデーションエラー |
| STAFF-096 | 水分タブで残り対応「破棄した」を選択可能 | 選択可能 |

### 13.9 実装計画

| ステップ | 内容 | 影響ファイル |
|----------|------|-------------|
| 29.1 | 設計ドキュメント更新 | STAFF_RECORD_FORM_SPEC.md, GOOGLE_CHAT_WEBHOOK_SPEC.md |
| 29.2 | タブ式UIコンポーネント追加 | StaffRecordDialog.tsx |
| 29.3 | デフォルトタブ選択ロジック | StaffRecordDialog.tsx |
| 29.4 | 水分量自動計算ロジック | StaffRecordDialog.tsx |
| 29.5 | タブ別API呼び出し分岐 | StaffRecordDialog.tsx |
| 29.6 | submitCareRecord Webhook連携 | functions/src/functions/submitCareRecord.ts |
| 29.7 | E2Eテスト追加 | staff-record-form.spec.ts |
| 29.8 | ローカルテスト・本番デプロイ | - |

### 13.10 後方互換性

- 既存のsnack記録フローに影響なし（食事タブとして継続）
- `recordType: 'hydration'` は既存APIで対応済み
- 飲み物以外のカテゴリでもタブ切り替えで水分記録可能（任意選択）
...
Phase 31でカテゴリが2つ（食べ物・飲み物）に簡素化されたことに伴い、
**タブ切り替え機能を廃止**し、カテゴリに応じてフォームを固定表示するよう変更。

### 14.2 変更内容

| 項目 | Before (Phase 29) | After (Phase 31) |
|------|-------------------|------------------|
| カテゴリ | 7種類（fruit, snack, drink, dairy, prepared, supplement, other） | 2種類（food, drink） |
| タブUI | クリック可能なタブボタン | 固定ヘッダー（切替不可） |
| フォーム選択 | ユーザーが任意に切替可能 | カテゴリで自動決定 |
...
```typescript
// getDefaultTab() は維持（旧カテゴリにも対応）
function getDefaultTab(category: string): 'meal' | 'hydration' {
  const migratedCategory = migrateCategory(category);
  return migratedCategory === 'drink' ? 'hydration' : 'meal';
}

// タブボタンを削除し、ヘッダー表示のみに変更
<div className="text-center py-2 px-4 font-medium border-b-2 border-primary">
...
- 既存データの旧カテゴリ（fruit, snack等）は `migrateCategory()` で自動変換
- drink → drink（飲み物）
- その他すべて → food（食べ物）
...
| 日付 | 内容 |
|------|------|
| 2025-12-21 | Phase 31: カテゴリ簡素化（7→2）、タブ固定化（切替不可）|
| 2025-12-21 | Phase 29: 特記事項の初期値仕様を明記（placeholder→defaultValue） |
| 2025-12-21 | Phase 29: タブ式UI設計追加（カテゴリ連動デフォルト選択、食事/水分タブ分離） |
| 2025-12-21 | Phase 29: 水分摂取量シート情報追加（シート名、カラム構造） |
| 2025-12-21 | Phase 29: Google Chat Webhookタグ仕様整理（タブ別ルール） |

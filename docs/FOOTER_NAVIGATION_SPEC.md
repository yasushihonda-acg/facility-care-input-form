...
> **作成日**: 2025-12-14
> **更新日**: 2025-12-20
> **関連**: [USER_ROLE_SPEC.md](./USER_ROLE_SPEC.md), [MEAL_INPUT_FORM_SPEC.md](./MEAL_INPUT_FORM_SPEC.md), [CHAT_INTEGRATION_SPEC.md](./CHAT_INTEGRATION_SPEC.md)
...
### 2. フッターナビゲーション（ロール別5タブ）

画面下部に固定表示のナビゲーションバーを配置。ロールに応じて異なるタブを表示。
Phase 18でチャットタブを追加し、未読バッジ表示に対応。
...
| チャット | 💬 | `/staff/chats` | 家族とのチャット（Phase 18追加）・未読バッジ表示 |
...
| チャット | 💬 | `/family/chats` | スタッフとのチャット（Phase 18追加）・未読バッジ表示 |
...
### Phase 18: チャット連携対応 ✅ 完了（2025-12-20）

1. ✅ `FooterNav.tsx` にチャットタブ追加（未読バッジ付き）
2. ✅ チャット一覧 `/staff/chats`, `/family/chats` ルート追加
3. ✅ `ItemChatPage.tsx` にFooterNav追加
4. ✅ `ChatListPage.tsx` にLayout使用でフッター表示

**重要**: チャット関連ページでもフッターナビゲーションを表示するため、以下の対応が必要:
- `ItemChatPage`: 入力エリアをフッター上（`bottom: calc(64px + env(safe-area-inset-bottom, 0))`）に配置
- `ChatListPage`: Layoutコンポーネントを使用（showFooter=true）

...
このアプリは**単一入居者専用**として設計されています。

| 項目 | 仕様 |
|------|------|
| 対象入居者数 | 1名のみ |
| `residentId` | 全ページで固定値 (`resident-001`) を使用 |
| マルチテナント | 非対応（将来の拡張予定なし） |

**コード上の影響**:
- `DEMO_RESIDENT_ID = 'resident-001'` が各ページで使用される
- `useCareItems`, `useConsumptionLogs` 等のフックは常に同一入居者のデータを取得
- 認証（AuthContext）には `residentId` を含めていない

この設計により、認証・データ取得がシンプルに保たれています。

...
> **Phase 25 追加** (2025-12-21)

### 概要

キーワード検索がスタッフ名だけでなく、すべてのカラム内容（特記事項、時間帯、数値など）を対象とする。

### 仕様

| 項目 | 値 |
|------|-----|
| 検索対象 | スタッフ名、タイムスタンプ、全データカラム値 |
| マッチング | 部分一致（大文字小文字を区別しない） |
| 表示 | マッチした件数をリアルタイム表示 |
...
|-----------|------|-----|
| `staffName` | 記録者名 | "田中" |
| `timestamp` | 記録日時 | "2025/12/20" |
| `data.*` | 全カラム値 | 特記事項、時間帯、数値など |

...
test.describe.skip('品物操作通知（Phase 55）【未実装】', () => {
  ...
  test('ITEM-NOTIFY-004: 変更通知に青バッジが表示される', async ({ page }) => {
    ...
    // 変更バッジが存在することを確認（青色）
    const updateBadge = page.locator('span.bg-blue-100').filter({ hasText: '変更' });
  ...
  test('ITEM-NOTIFY-006: 品物操作通知が他のタスクより上に表示される', async ({ page }) => {
    await page.goto('/demo/staff/notes');
    await page.waitForLoadState('networkidle');
    ...
    // 品物更新通知セクションの位置を取得
    const itemNotifySection = page.locator('text=品物更新通知');
    const otherSection = page.locator('text=その他の依頼');

    // 両方が表示されていることを確認
    await expect(itemNotifySection).toBeVisible();
    await expect(otherSection).toBeVisible();

    // 品物更新通知がその他の依頼より上に表示されていることを確認
    const itemNotifyY = await itemNotifySection.boundingBox().then(box => box?.y ?? 0);
    const otherY = await otherSection.boundingBox().then(box => box?.y ?? 0);
    expect(itemNotifyY).toBeLessThan(otherY);
  });
  ...
  test('ITEM-NOTIFY-008: 品物操作タスクカードに左ボーダーが表示される', async ({ page }) => {
    await page.goto('/demo/staff/notes');
    await page.waitForLoadState('networkidle');
  ...
  test('ITEM-NOTIFY-010: 品物操作タスクに登録日時が表示される', async ({ page }) => {
    await page.goto('/demo/staff/notes');
    await page.waitForLoadState('networkidle');

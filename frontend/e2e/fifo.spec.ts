...
test.describe('FIFO-001: 品物一覧表示順序', () => {
  ...
  test('期限切れ品物が先頭に表示される', async ({ page }) => {
    ...
    // ヨーグルト（期限切れ: demo-item-012）が表示されていることを確認
    // FIFOソートでは期限切れが最優先で表示される
    const expiredItem = page.locator('text=ヨーグルト');
...
test.describe('FIFO-004: 品物詳細でのSameItemAlert', () => {
  test('りんご詳細ページで同一品物アラートが表示される', async ({ page }) => {
    // demo-item-003（りんご）の詳細ページにアクセス
    // demo-item-013も同じ「りんご」なのでSameItemAlertが表示されるはず
  ...
  test('バナナ詳細ページで同一品物アラートが表示される', async ({ page }) => {
    // demo-item-001（バナナ）の詳細ページにアクセス
    // demo-item-014も同じ「バナナ」なのでSameItemAlertが表示されるはず
  ...
  test('SameItemAlertに「先に消費推奨」マークが表示される', async ({ page }) => {
    // demo-item-003（りんご、期限+5日）の詳細ページにアクセス
    // demo-item-013（りんご、期限+2日）の方が期限が近いので「先に消費推奨」が表示される
    await page.goto('/demo/family/items/demo-item-003');
...
test.describe('FIFO表示順序の正確性', () => {
  ...
  test('FIFOソートでデモデータが正しくソートされている', async ({ page }) => {
    ...
    // 期限が近い品物（バナナ: +0日、柿: +1日）が上位に表示されていることを確認
    // 正確な順序のテストは実装詳細に依存するため、表示の確認のみ
    const pageContent = await page.locator('main').textContent();
    expect(pageContent).toContain('バナナ');
    expect(pageContent).toContain('柿');
  });
});

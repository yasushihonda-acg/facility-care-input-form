...
test.describe('画像一括登録機能', () => {
  ...
  test('IMG-002: デフォルトで画像タブが選択されている', async ({ page }) => {
    // 画像タブがアクティブ（緑系の色）
  ...
  test('IMG-004: 画像タブで対応形式が表示される', async ({ page }) => {
    // デフォルトで画像タブが選択されている（Phase 69）

    // 対応形式の説明が表示される
    await expect(page.getByText(/JPEG, PNG, WebP/)).toBeVisible({ timeout: 10000 });
    await expect(page.getByText(/各5MB以下/)).toBeVisible({ timeout: 10000 });
    await expect(page.getByText(/最大5枚/)).toBeVisible({ timeout: 10000 });
  });
  ...
  test('IMG-006: デモモードで画像解析がモックデータを返す', async ({ page }) => {
    ...
    const fileInput = page.locator('input[type="file"][multiple]:not([capture])');
    ...
    // 画像が選択されたことを確認（プレビューが表示される）
    await expect(page.getByText('1枚選択中')).toBeVisible({ timeout: 10000 });

    // 解析ボタンをクリック（Phase 69: 複数画像対応で解析は明示的なボタンクリックに変更）
    await page.getByRole('button', { name: /枚の画像を解析する/ }).click();

    // 解析中の表示を待つ
    await expect(page.getByText(/枚の画像を解析中/)).toBeVisible({ timeout: 10000 });
  ...
  test('IMG-009: 複数画像をアップロードできる', async ({ page }) => {
    // デフォルトで画像タブが選択されている（Phase 69）
    ...
    // 複数ファイルをアップロード
    await fileInput.setInputFiles([
      { name: 'schedule1.png', mimeType: 'image/png', buffer: dummyPngBuffer },
      { name: 'schedule2.png', mimeType: 'image/png', buffer: dummyPngBuffer },
    ]);

    // 2枚選択中と表示される
    await expect(page.getByText('2枚選択中')).toBeVisible({ timeout: 10000 });

    // 解析ボタンが表示される
    await expect(page.getByRole('button', { name: /2枚の画像を解析する/ })).toBeVisible({ timeout: 10000 });

    // ヒントメッセージが表示される
    await expect(page.getByText('複数画像での読み取り')).toBeVisible({ timeout: 10000 });
  });
});

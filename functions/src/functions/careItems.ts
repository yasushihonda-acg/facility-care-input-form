...
/**
 * 品物操作タスクを自動作成（Phase 55）
 * 家族が品物を登録・更新・削除した際に、スタッフ通知用のタスクを作成
 * 24時間後に自動削除される想定
 *
 * @param actionType - 操作タイプ（item_created, item_updated, item_deleted）
 * @param itemName - 品物名
 * @param residentId - 入居者ID
 * @param relatedItemId - 関連する品物ID（削除時はundefinedも可）
 * @param description - タスクの説明（変更内容など）
 */
async function createItemActionTask(
  actionType: TaskType,
  itemName: string,
  residentId: string,
  ...
  const titlePrefix = titleMap[actionType] || "";
  ...
  functions.logger.info("createItemActionTask success", {taskId, actionType, itemName});
}

/**
 * デフォルトの説明文を取得
 */
function getDefaultDescription(actionType: TaskType): string {
  switch (actionType) {
  case "item_created":
    return "家族が新しい品物を登録しました。";
  case "item_updated":
    return "家族が品物情報を更新しました。";
  case "item_deleted":
    return "家族が品物を削除しました。";
  default:
    return "";
...
async function submitCareItemHandler(
  ...
  try {
    ...
    // Phase 55: 品物操作タスクを自動作成（非同期・エラーでも処理続行）
    try {
      await createItemActionTask(
        "item_created",
        item.itemName,
        residentId,
        docRef.id
      ...
      functions.logger.warn("submitCareItem task creation failed:", taskError);
    }
...
async function updateCareItemHandler(
  ...
  try {
    ...
    // Phase 55: 品物操作タスクを自動作成（非同期・エラーでも処理続行）
    try {
      // 更新後のデータを取得（通知で既に取得済みの場合はキャッシュ利用）
      const currentDoc = await docRef.get();
      const currentItem = currentDoc.data() as CareItem;
      await createItemActionTask(
        "item_updated",
        currentItem.itemName,
        currentItem.residentId,
        itemId,
        "家族が品物情報を更新しました。"
      );
    } catch (taskError) {
      functions.logger.warn("updateCareItem task creation failed:", taskError);
    }
  ...
  try {
    ...
    try {
      await createItemActionTask(
        ...
        deletedItem.residentId,
        undefined, // 削除済みなのでrelatedItemIdはなし
        "家族が品物を削除しました。"
      );
    } catch (taskError) {
      functions.logger.warn("deleteCareItem task creation failed:", taskError);
    }
